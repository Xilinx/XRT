...
//    b[i] = (rand() / (int)RAND_MAX);
  ...
  err = clGetDeviceIDs(platform_id, fpga ? CL_DEVICE_TYPE_ACCELERATOR : CL_DEVICE_TYPE_CPU, 0, NULL, &num_devices);
  if (err != CL_SUCCESS)
  {
  ...
  struct timespec clstart,clend;
  clock_gettime(CLOCK_MONOTONIC,&clstart);
  ...
  clock_gettime(CLOCK_MONOTONIC,&clend);
  double clelapsed = time_elapsed(clstart,clend);
...
//    printf("%0.2f ",b[i]);
  ...
  //
  struct timespec swstart,swend;
  clock_gettime(CLOCK_MONOTONIC,&swstart);
  correct = 0;
  for(i = 0; i < DATA_SIZE; i++)
  {
  ...
  clock_gettime(CLOCK_MONOTONIC,&swend);
  double swelapsed = time_elapsed(swstart,swend);
