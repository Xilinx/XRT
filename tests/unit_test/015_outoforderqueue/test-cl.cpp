...
//    b[i] = (rand() / (int)RAND_MAX);
  ...
  //
  err = clEnqueueWriteBuffer(commands, input_a, CL_TRUE, 0, sizeof(int) * DATA_SIZE, a, 0, NULL, &writeevents[0]);
  if (err != CL_SUCCESS)
  {
  ...
  // Write our data set into the input array in device memory 
  //
  err = clEnqueueWriteBuffer(commands, input_b, CL_TRUE, 0, sizeof(int) * DATA_SIZE, b, 0, NULL, &writeevents[1]);
  if (err != CL_SUCCESS)
  {
  ...
  err = clEnqueueNDRangeKernel(commands, kernel, 2, NULL, 
                               (size_t*)&global, (size_t*)&local, 2, writeevents, &multevent);
  if (err)
  {
  ...
  //
  err = clEnqueueReadBuffer( commands, output, CL_TRUE, 0, sizeof(int) * DATA_SIZE, results, 1, &multevent, &readevent );  
  if (err != CL_SUCCESS)
  {
