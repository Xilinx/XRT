...
int32_t
directoryOrError(const fs::path& path)
{
...
load_libxrt()
{
    ...
    fs::path xma2plugin_lib(xrt / "lib/libxma2plugin.so");
    ...
    if (!isEmulationMode()) {
        fs::path p1(xrt / "lib/libxrt_core.so");
        if (isDLL(p1)) {
        ...
        fs::path p2(xrt / "lib/libxrt_aws.so");
        if (isDLL(p2)) {
    ...
    if (is_sw_emulation()) {
        ...
        fs::path p2(xrt / "lib/libxrt_swemu.so");
    ...
    fs::path p1(xrt / "lib/libxrt_hwemu.so");
