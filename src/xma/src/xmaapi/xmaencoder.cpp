...
xma_enc_session_create(XmaEncoderProperties *enc_props)
{
    ...
    if (enc_props->ddr_bank_index < 0) {
        ...
        if (enc_props->ddr_bank_index >= MAX_DDR_MAP) {
            xma_logmsg(XMA_INFO_LOG, XMA_ENCODER_MOD,
                        "User selected ddr_bank is outside the valid range. XMA session ddr_bank: %d\n", enc_session->base.hw_session.kernel_info->ddr_bank);
        } else {
            if (enc_session->base.hw_session.kernel_info->ip_ddr_mapping[enc_props->ddr_bank_index]) {
                xma_logmsg(XMA_INFO_LOG, XMA_ENCODER_MOD,
                            "Will use user selected ddr bank. XMA (automatic) ddr_bank: %d\n", enc_session->base.hw_session.kernel_info->ddr_bank);
                enc_session->base.hw_session.kernel_info->ddr_bank = enc_props->ddr_bank_index;
                xma_logmsg(XMA_INFO_LOG, XMA_ENCODER_MOD,
