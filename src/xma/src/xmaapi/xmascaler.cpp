...
xma_scaler_session_create(XmaScalerProperties *sc_props)
{
    ...
    if (sc_props->ddr_bank_index < 0) {
        ...
        if (sc_props->ddr_bank_index >= MAX_DDR_MAP) {
            xma_logmsg(XMA_INFO_LOG, XMA_SCALER_MOD,
                        "User selected ddr_bank is outside the valid range. XMA session ddr_bank: %d\n", sc_session->base.hw_session.kernel_info->ddr_bank);
        } else {
            if (sc_session->base.hw_session.kernel_info->ip_ddr_mapping[sc_props->ddr_bank_index]) {
                xma_logmsg(XMA_INFO_LOG, XMA_SCALER_MOD,
                            "Will use user selected ddr bank. XMA (automatic) ddr_bank: %d\n", sc_session->base.hw_session.kernel_info->ddr_bank);
                sc_session->base.hw_session.kernel_info->ddr_bank = sc_props->ddr_bank_index;
                xma_logmsg(XMA_INFO_LOG, XMA_SCALER_MOD,
