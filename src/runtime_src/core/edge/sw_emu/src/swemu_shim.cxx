...
namespace xcl_swemu_hal {

  std::map<unsigned int, swemu_shim*> devices;
  unsigned int swemu_shim::mBufferCount = 0;
  unsigned int GraphType::mGraphHandle = 0;
  std::map<int, std::tuple<std::string,int,void*> > swemu_shim::mFdToFileNameMap;
  bool swemu_shim::mFirstBinary = true;
  const unsigned swemu_shim::TAG = 0X586C0C6C; // XL OpenCL X->58(ASCII), L->6C(ASCII), O->0 C->C L->6C(ASCII);
  const unsigned swemu_shim::CONTROL_AP_START = 1;
  const unsigned swemu_shim::CONTROL_AP_DONE  = 2;
  const unsigned swemu_shim::CONTROL_AP_IDLE  = 4;
  const unsigned swemu_shim::CONTROL_AP_CONTINUE = 0x10;
  std::map<std::string, std::string> swemu_shim::mEnvironmentNameValueMap(xclemulation::getEnvironmentByReadingIni());
#define PRINTENDFUNC if (mLogStream.is_open()) mLogStream << __func__ << " ended " << std::endl;
  ...
  bool initRemotePortMap(std::string fpgaDevice)
  {
    ...
    xcl_swemu_hal::isRemotePortMapped = true;
    return true;
  }
  ...
  swemu_shim::swemu_shim(unsigned int deviceIndex, xclDeviceInfo2 &info, std::list<xclemulation::DDRBank>& DDRBankList, bool _unified, bool _xpr, FeatureRomHeader& fRomHeader)
    :mTag(TAG)
  ...
  size_t swemu_shim::alloc_void(size_t new_size)
  {
    if (buf_size == 0)
    {
      buf = malloc(new_size);
  ...
  void swemu_shim::initMemoryManager(std::list<xclemulation::DDRBank>& DDRBankList)
  {
...
//private
  bool swemu_shim::isGood() const
  {
    // TODO: Add sanity check for card state
    return true;
  }

  swemu_shim *swemu_shim::handleCheck(void *handle)
  {
    // Sanity checks
    if (!handle)
      return 0;
    if (*(unsigned *)handle != TAG)
      return 0;
    if (!((swemu_shim *)handle)->isGood()) {
      return 0;
    }
    return (swemu_shim *)handle;
  }

  static void saveDeviceProcessOutputs()
  {
    std::map<unsigned int, swemu_shim*>::iterator start = devices.begin();
    std::map<unsigned int, swemu_shim*>::iterator end = devices.end();
    for(; start != end; start++)
    {
      swemu_shim* handle = (*start).second;
      if(!handle)
        continue;
  ...
  int swemu_shim::dumpXML(const xclBin* header, std::string& fileLocation)
  {
  ...
  bool swemu_shim::parseIni(unsigned int& debugPort)
  {
  ...
  void swemu_shim::launchDeviceProcess(bool debuggable, std::string& binaryDirectory)
  {
  ...
  int swemu_shim::xclLoadXclBin(const xclBin *header)
  {
    if (mLogStream.is_open()) mLogStream << __func__ << " begin " << std::endl;
    std::string xclBinName = "";
    if (!xcl_swemu_hal::validateXclBin(header, xclBinName, mDeviceProcessInQemu, mFpgaDevice)) {
   ...
   bool swemu_shim::isValidCu(uint32_t cu_index) {  
  ...
  uint64_t swemu_shim::getCuAddRange(uint32_t cu_index) {
  ...
  bool swemu_shim::isValidOffset(uint32_t offset, uint64_t cuAddRange) { 
  ...
  int swemu_shim::xclRegRW(bool rd, uint32_t cu_index, uint32_t offset, uint32_t *datap)
  {
    if (mLogStream.is_open())
  ...
  int swemu_shim::xclRegRead(uint32_t cu_index, uint32_t offset, uint32_t *datap)
  {
    if (mLogStream.is_open()) {
      mLogStream << __func__ << ", " << std::this_thread::get_id() << ", " << "CU Idx : " << cu_index << " Offset : " << offset << " Datap : " << (*datap)  << std::endl;
    }
  ...
  int swemu_shim::xclRegWrite(uint32_t cu_index, uint32_t offset, uint32_t data)
  {
    if (mLogStream.is_open()) {
  ...
  bool swemu_shim::isAieEnabled(const xclBin* header)
  {
    bool aieFlag = false;
  ...
  void swemu_shim::socketConnection(bool isTCPSocket) {
  ...
  int swemu_shim::xclLoadXclBinNewFlow(const xclBin *header)
  {
    if (mLogStream.is_open()) mLogStream << __func__ << " begin " << std::endl;
    ...
    std::thread tcpSockThread = std::thread(&swemu_shim::socketConnection, this, true);

    bool simDontRun = xclemulation::config::getInstance()->isDontRun();
    if (!simDontRun) {
      if (!xcl_swemu_hal::isRemotePortMapped) {
        xcl_swemu_hal::initRemotePortMap(mFpgaDevice);
      }
      //Send the LoadXclBin
      ...
      for (unsigned int i = 0; i < length; i += 4) {
        ...
        memcpy(((char*)(xcl_swemu_hal::remotePortMappedPointer)) + i, buff + i, copySize);
      }
      //Send the end of packet
      char cPacketEndChar = PL_OCL_PACKET_END_MARKER;
      memcpy((char*)(xcl_swemu_hal::remotePortMappedPointer), &cPacketEndChar, 1);
    }
  ...
  int swemu_shim::xclGetDeviceInfo2(xclDeviceInfo2 *info)
  {
  ...
  void swemu_shim::launchTempProcess()
  {
    std::string binaryDirectory("");
  ...
  uint64_t swemu_shim::xclAllocDeviceBuffer(size_t size)
  {
  ...
  uint64_t swemu_shim::xclAllocDeviceBuffer2(size_t& size, xclMemoryDomains domain, unsigned flags,bool noHostMemory,std::string &sFileName)
  {
    if (mLogStream.is_open()) {
  ...
  void swemu_shim::xclFreeDeviceBuffer(uint64_t offset)
  {
    if (mLogStream.is_open()) {
  ...
  size_t swemu_shim::xclWrite(xclAddressSpace space, uint64_t offset, const void *hostBuf, size_t size)
  {
  ...
  size_t swemu_shim::xclRead(xclAddressSpace space, uint64_t offset, void *hostBuf, size_t size)
  {
    std::lock_guard<std::mutex> lk(mApiMtx);
    if (mLogStream.is_open()) {
  ...
  size_t swemu_shim::xclCopyBufferHost2Device(uint64_t dest, const void *src, size_t size, size_t seek)
  {
    if (mLogStream.is_open()) {
  ...
  size_t swemu_shim::xclCopyBufferDevice2Host(void *dest, uint64_t src, size_t size, size_t skip)
  {
    if (mLogStream.is_open()) {
      mLogStream << __func__ << ", " << std::this_thread::get_id() << ", " << dest << ", "
  ...
  void swemu_shim::xclOpen(const char* logfileName)
  {
  ...
  void swemu_shim::fillDeviceInfo(xclDeviceInfo2* dest, xclDeviceInfo2* src)
  {
  ...
  void swemu_shim::saveDeviceProcessOutput()
  {
    if(!sock)
      return;
  ...
  void swemu_shim::resetProgram(bool callingFromClose)
  {
  ...
  swemu_shim::~swemu_shim()
  {
    ...
    if (xcl_swemu_hal::isRemotePortMapped) {
      ...
      uint32_t iLength;
      memcpy((char*)(xcl_swemu_hal::remotePortMappedPointer), (char*)cmd->generateBuffer(&iLength), iLength);
      //Send the end of packet
      char cPacketEndChar = PL_OCL_PACKET_END_MARKER;
      memcpy((char*)(xcl_swemu_hal::remotePortMappedPointer), &cPacketEndChar, 1);
    }
...
xclemulation::drm_xocl_bo* swemu_shim::xclGetBoByHandle(unsigned int boHandle)
{
...
inline unsigned short swemu_shim::xocl_ddr_channel_count()
{
  return mDeviceInfo.mDDRBankCount;
}

inline unsigned long long swemu_shim::xocl_ddr_channel_size()
{
  return 0;
}

int swemu_shim::xclGetBOProperties(unsigned int boHandle, xclBOProperties *properties)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
  {
...
/******************************** xclAllocBO *********************************************/
uint64_t swemu_shim::xoclCreateBo(xclemulation::xocl_create_bo* info)
{
...
unsigned int swemu_shim::xclAllocBO(size_t size, int unused, unsigned flags)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
  {
...
/******************************** xclAllocUserPtrBO ************************************/
unsigned int swemu_shim::xclAllocUserPtrBO(void *userptr, size_t size, unsigned flags)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
  {
...
/******************************** xclExportBO *******************************************/
int swemu_shim::xclExportBO(unsigned int boHandle)
{
  if (mLogStream.is_open())
  {
    mLogStream << __func__ << ", " << std::this_thread::get_id() << ", " << std::hex << boHandle << std::endl;
...
/******************************** xclImportBO *******************************************/
unsigned int swemu_shim::xclImportBO(int boGlobalHandle, unsigned flags)
{
  //TODO
  if (mLogStream.is_open())
  {
...
/******************************** xclCopyBO *******************************************/
int swemu_shim::xclCopyBO(unsigned int dst_boHandle, unsigned int src_boHandle, size_t size, size_t dst_offset, size_t src_offset)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  //TODO
  if (mLogStream.is_open())
...
/******************************** xclMapBO *********************************************/
void *swemu_shim::xclMapBO(unsigned int boHandle, bool write)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
  {
...
int swemu_shim::xclUnmapBO(unsigned int boHandle, void* addr)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
...
/******************************** xclSyncBO *******************************************/
int swemu_shim::xclSyncBO(unsigned int boHandle, xclBOSyncDirection dir, size_t size, size_t offset)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
  {
...
/******************************** xclFreeBO *******************************************/
void swemu_shim::xclFreeBO(unsigned int boHandle)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
  {
...
/******************************** xclWriteBO *******************************************/
size_t swemu_shim::xclWriteBO(unsigned int boHandle, const void *src, size_t size, size_t seek)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
  {
...
/******************************** xclReadBO *******************************************/
size_t swemu_shim::xclReadBO(unsigned int boHandle, void *dst, size_t size, size_t skip)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
  {
 ...
 * xclCreateWriteQueue()
 */
int swemu_shim::xclCreateWriteQueue(xclQueueContext *q_ctx, uint64_t *q_hdl)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
    mLogStream << __func__ << ", " << std::this_thread::get_id() << std::endl;
 ...
 * xclCreateReadQueue()
 */
int swemu_shim::xclCreateReadQueue(xclQueueContext *q_ctx, uint64_t *q_hdl)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
  {
 ...
 * xclDestroyQueue()
 */
int swemu_shim::xclDestroyQueue(uint64_t q_hdl)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
  {
 ...
 * xclWriteQueue()
 */
ssize_t swemu_shim::xclWriteQueue(uint64_t q_hdl, xclQueueRequest *wr)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
  {
 ...
 * xclReadQueue()
 */
ssize_t swemu_shim::xclReadQueue(uint64_t q_hdl, xclQueueRequest *rd)
{
  if (mLogStream.is_open())
  {
    mLogStream << __func__ << ", " << std::this_thread::get_id() << std::endl;
 ...
 * xclPollCompletion
 */
int swemu_shim::xclPollCompletion(int min_compl, int max_compl, xclReqCompletion *comps, int* actual, int timeout)
{
  if (mLogStream.is_open())
  {
 ...
 * xclAllocQDMABuf()
 */
void * swemu_shim::xclAllocQDMABuf(size_t size, uint64_t *buf_hdl)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
  {
 ...
 * xclFreeQDMABuf()
 */
int swemu_shim::xclFreeQDMABuf(uint64_t buf_hdl)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
  {
 ...
 * xclLogMsg()
 */
int swemu_shim::xclLogMsg(xclDeviceHandle handle, xrtLogMsgLevel level, const char* tag, const char* format, va_list args1)
{
...
* xclOpenContext
*/
int swemu_shim::xclOpenContext(const uuid_t xclbinId, unsigned int ipIndex, bool shared) const
{
  return 0;
}

int swemu_shim::xclOpenContext(uint32_t slot, const uuid_t xclbinId, const char* cuname, bool shared) const
{
  return 0;
}
...
* xclExecWait
*/
int swemu_shim::xclExecWait(int timeoutMilliSec)
{
  //unsigned int tSec = 0;
...
* xclExecBuf
*/
int swemu_shim::xclExecBuf(unsigned int cmdBO)
{
  if (mLogStream.is_open())
  {
...
* xclCloseContext
*/
int swemu_shim::xclCloseContext(const uuid_t xclbinId, unsigned int ipIndex) const
{
  return 0;
}

//Get CU index from IP_LAYOUT section for corresponding kernel name
int swemu_shim::xclIPName2Index(const char *name)
{
...
*/
int swemu_shim::xrtGraphInit(void * gh) {
  bool ack = false;
  auto ghPtr = (xcl_swemu_hal::GraphType*)gh;
  if (!ghPtr)
    return -1;
...
*/
int swemu_shim::xrtGraphRun(void * gh, uint32_t iterations) {
  bool ack = false;
  auto ghPtr = (xcl_swemu_hal::GraphType*)gh;
  if (!ghPtr)
    return -1;
...
*/
int swemu_shim::xrtGraphWait(void * gh) {
  bool ack = false;
  auto ghPtr = (xcl_swemu_hal::GraphType*)gh;
  if (!ghPtr)
    return -1;
...
*                   cycle, stop the graph immediateley.
*/
int swemu_shim::xrtGraphTimedWait(void * gh, uint64_t cycle) {
  bool ack = false;
  auto ghPtr = (xcl_swemu_hal::GraphType*)gh;
  if (!ghPtr)
    return -1;
...
*/
int swemu_shim::xrtGraphEnd(void * gh) {
  bool ack = false;
  auto ghPtr = (xcl_swemu_hal::GraphType*)gh;
  if (!ghPtr)
    return -1;
...
* forever or graph that has multi-rate core(s).
*/
int swemu_shim::xrtGraphTimedEnd(void * gh , uint64_t cycle) {
  bool ack = false;
  auto ghPtr = (xcl_swemu_hal::GraphType*)gh;
  if (!ghPtr)
    return -1;
...
*/
int swemu_shim::xrtGraphResume(void * gh) {
  bool ack = false;
  auto ghPtr = (xcl_swemu_hal::GraphType*)gh;
  if (!ghPtr)
    return -1;
...
*/
int swemu_shim::xrtGraphUpdateRTP(void * gh, const char *hierPathPort, const char *buffer, size_t size) {
  auto ghPtr = (xcl_swemu_hal::GraphType*)gh;
  if (!ghPtr)
    return -1;
  auto graphhandle = ghPtr->getGraphHandle();
...
*       being copied to.
*/
int swemu_shim::xrtGraphReadRTP(void * gh, const char *hierPathPort, char *buffer, size_t size) {
  auto ghPtr = (xcl_swemu_hal::GraphType*)gh;
  if (!ghPtr)
    return -1;
  auto graphhandle = ghPtr->getGraphHandle();
...
int swemu_shim::xrtSyncBOAIENB(xrt::bo& bo, const char *gmioname, enum xclBOSyncDirection dir, size_t size, size_t offset)
{
  bool ack = false;
  if (!gmioname)
    return -1;
...
* Return:          0 on success, or appropriate error number.
*/
int swemu_shim::xrtGMIOWait(const char *gmioname)
{
  bool ack = false;
  if (!gmioname)
    return -1;
