...
xclDeviceHandle xclOpen(unsigned deviceIndex, const char *logfileName, xclVerbosityLevel level)
{
  ...
  xcl_swemu_hal::swemu_shim *handle = nullptr;
  ...
  {
    handle = new xcl_swemu_hal::swemu_shim(deviceIndex,info,DDRBankList,false,false,fRomHeader);
    bDefaultDevice = true;
...
unsigned xclProbe()
{
  ...
  for(auto &it: devicesInfo)
  {
    ...
    xcl_swemu_hal::swemu_shim *handle = new xcl_swemu_hal::swemu_shim(deviceIndex,info,DDRBankList, bUnified, bXPR, fRomHeader);
    xcl_swemu_hal::devices[deviceIndex++] = handle;
  }
