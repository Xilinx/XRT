...
namespace xcl_swemu_hal {
  ...
  void SWScheduler::cu_continue(struct xocl_cu *xcu)
  {
    ...
    mParent->xclWrite(XCL_ADDR_KERNEL_CTRL, xcu->base + xcu->addr, (void*)&swemu_shim::CONTROL_AP_CONTINUE,4);

    // in ert_poll mode acknowlegde done to ERT
    if (xcu->polladdr && xcu->run_cnt) {
      mParent->xclWrite(XCL_ADDR_KERNEL_CTRL,xcu->base + xcu->polladdr, (void*)&swemu_shim::CONTROL_AP_CONTINUE,4);
    }
  }
  ...
  void SWScheduler::cu_poll(struct xocl_cu *xcu)
  {
    ...
    if (xcu->run_cnt && (xcu->ctrlreg & (swemu_shim::CONTROL_AP_DONE | swemu_shim::CONTROL_AP_IDLE)))
    {
      ++xcu->done_cnt;
      --xcu->run_cnt;
      cu_continue(xcu);
  ...
  {
    PRINTSTARTFUNC
    if ((xcu->ctrlreg & swemu_shim::CONTROL_AP_START) || (!xcu->dataflow && xcu->run_cnt))
      cu_poll(xcu);

    bool bReady = xcu->dataflow ? !(xcu->ctrlreg & swemu_shim::CONTROL_AP_START) : xcu->run_cnt == 0;
    return bReady;
  }
  ...
  bool SWScheduler::cu_start(struct xocl_cu *xcu, struct xocl_cmd *xcmd)
  {
    ...
    xcu->ctrlreg |= swemu_shim::CONTROL_AP_START;
    const_cast<uint32_t*>(regmap)[0] = swemu_shim::CONTROL_AP_START;
    //mParent->xclWrite(XCL_ADDR_KERNEL_CTRL, xcu->base + xcu->addr,(void*)& swemu_shim::CONTROL_AP_START, 4);
    mParent->xclWrite(XCL_ADDR_KERNEL_CTRL, xcu->base + xcu->addr, (void*)(regmap), size*4);
    // in ert poll mode request ERT to poll CU
    if (xcu->polladdr) {
      mParent->xclWrite(XCL_ADDR_KERNEL_CTRL, xcu->base + xcu->polladdr, (void*)& swemu_shim::CONTROL_AP_START, 4);
    }
  ...
  SWScheduler::SWScheduler(swemu_shim* _parent)
  {
    PRINTSTARTFUNC
    mParent = _parent;
