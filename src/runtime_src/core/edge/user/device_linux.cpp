...
struct aie_skd_xclbin
{
  ...
  get(const xrt_core::device* device, key_type key, const std::any& skd_uuid_ptr, const std::any& skd_axlf_ptr)
  {
    ...
    result_type ret = 0;
    /* Get the Axlf Buffer */
    ret = xclgetAIESkdXclbin(device->get_user_handle(), std::any_cast<xrt_core::query::aie_skd_xclbin::skd_uuid_ptr_type>(skd_uuid_ptr),
                    std::any_cast<xrt_core::query::aie_skd_xclbin::skd_axlf_ptr_type>(skd_axlf_ptr));
#else
    throw xrt_core::error(-EINVAL, "AIE is not enabled for this device");
#endif
    return ret;
...
struct skd_axlf_size
{
  ...
  {
#if defined(XRT_ENABLE_AIE)
    result_type axlf_size = 0;
    /* Read the Axlf size */
    axlf_size = xclgetAIESkdAxlfSize(device->get_user_handle(), std::any_cast<xrt_core::query::skd_axlf_size::skd_uuid_ptr_type>(skd_uuid_ptr));
#else
    throw xrt_core::error(-EINVAL, "AIE is not enabled for this device");
#endif
    return axlf_size;
...
initialize_query_table()
{
  ...
  emplace_func2_request<query::aie_skd_xclbin,          aie_skd_xclbin>();
