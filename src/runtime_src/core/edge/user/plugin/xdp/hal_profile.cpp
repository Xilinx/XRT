...
//      throw std::runtime_error("Both profile=true and profile_api=true set in xrt.ini config. Currently, these flows are not supported to work together. Hence, HAL profiling will not be available in this run. Please set only one of the configuration depending on type of application and re-run");
