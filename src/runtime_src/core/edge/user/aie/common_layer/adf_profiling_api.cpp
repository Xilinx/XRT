...
err_code profiling::profile_stream_running_to_idle_cycles(shim_config shimConfig, std::vector<std::shared_ptr<xaiefal::XAieRsc>>& acquiredResources)
{
    ...
    driverStatus |= XAie_EventSelectStrmPort(config_manager::s_pDevInst, tileLoc, (u8)eventPortId, (slaveOrMaster == 0 ? XAIE_STRMSW_SLAVE : XAIE_STRMSW_MASTER), SOUTH, (u8)streamPortId);
    ...
    driverStatus |= XAie_PerfCounterControlSet(config_manager::s_pDevInst, tileLoc, XAIE_PL_MOD, (u8)counterId, COMMON_XAIETILE_EVENT_SHIM_PORT_RUNNING[eventPortId], COMMON_XAIETILE_EVENT_SHIM_PORT_IDLE[eventPortId]);
...
err_code profiling::profile_stream_start_to_transfer_complete_cycles(shim_config shimConfig, uint32_t numBytes, std::vector<std::shared_ptr<xaiefal::XAieRsc>>& acquiredResources)
{
    int shimColumn = shimConfig.shimColumn;
    int streamPortId = shimConfig.streamPortId;
    u8 slaveOrMaster = shimConfig.slaveOrMaster; //0:slave, 1:master
    ...
    driverStatus |= XAie_PerfCounterEventValueSet(config_manager::s_pDevInst, tileLoc, XAIE_PL_MOD, (u8)counterId1, (u32)(numBytes / 4));
    ...
    driverStatus |= XAie_PerfCounterControlSet(config_manager::s_pDevInst, tileLoc, XAIE_PL_MOD, (u8)counterId0, COMMON_XAIETILE_EVENT_SHIM_PORT_RUNNING[eventPortId], XAIE_EVENT_PERF_CNT_1_PL);
    debugMsg(static_cast<std::stringstream &&>(std::stringstream() << "XAie_PerfCounterControlSet: col " << (int)tileLoc.Col
    ...
    driverStatus |= XAie_PerfCounterControlSet(config_manager::s_pDevInst, tileLoc, XAIE_PL_MOD, (u8)counterId1, COMMON_XAIETILE_EVENT_SHIM_PORT_RUNNING[eventPortId], COMMON_XAIETILE_EVENT_SHIM_PORT_RUNNING[eventPortId]);
    debugMsg(static_cast<std::stringstream &&>(std::stringstream() << "XAie_PerfCounterControlSet: col " << (int)tileLoc.Col
...
err_code profiling::profile_stream_running_event_count(shim_config shimConfig, std::vector<std::shared_ptr<xaiefal::XAieRsc>>& acquiredResources)
{
    int shimColumn = shimConfig.shimColumn;
    int streamPortId = shimConfig.streamPortId;
    u8 slaveOrMaster = shimConfig.slaveOrMaster; //0:slave, 1:master
    ...
    driverStatus |= XAie_PerfCounterControlSet(config_manager::s_pDevInst, tileLoc, XAIE_PL_MOD, (u8)counterId, COMMON_XAIETILE_EVENT_SHIM_PORT_RUNNING[eventPortId], COMMON_XAIETILE_EVENT_SHIM_PORT_RUNNING[eventPortId]);
    debugMsg(static_cast<std::stringstream &&>(std::stringstream() << "XAie_PerfCounterControlSet: col " << (int)tileLoc.Col
...
err_code profiling::profile_start_time_difference_btw_two_streams(shim_config shimConfig1, shim_config shimConfig2, std::vector<std::shared_ptr<xaiefal::XAieRsc>>& acquiredResources)
{
    ...
    {
        driverStatus |= XAie_EventSelectStrmPort(config_manager::s_pDevInst, tileLoc1, (u8)eventPortId1, (slaveOrMaster1 == 0 ? XAIE_STRMSW_SLAVE : XAIE_STRMSW_MASTER), SOUTH, (u8)streamPortId1);
        ...
        driverStatus |= XAie_PerfCounterControlSet(config_manager::s_pDevInst, tileLoc1, XAIE_PL_MOD, (u8)counterId1, XAIE_EVENT_USER_EVENT_0_PL, COMMON_XAIETILE_EVENT_SHIM_PORT_RUNNING[eventPortId1]); //see Table 6-17
        ...
        driverStatus |= XAie_EventSelectStrmPort(config_manager::s_pDevInst, tileLoc2, (u8)eventPortId2, (slaveOrMaster2 == 0 ? XAIE_STRMSW_SLAVE : XAIE_STRMSW_MASTER), SOUTH, (u8)streamPortId2);
        ...
        driverStatus |= XAie_PerfCounterControlSet(config_manager::s_pDevInst, tileLoc2, XAIE_PL_MOD, (u8)counterId2, XAIE_EVENT_USER_EVENT_0_PL, COMMON_XAIETILE_EVENT_SHIM_PORT_RUNNING[eventPortId2]); //see Table 6-17
        ...
        driverStatus |= XAie_EventGenerate(config_manager::s_pDevInst, tileLoc1, XAIE_PL_MOD, XAIE_EVENT_USER_EVENT_0_PL);
    ...
    else
    {
        ...
        for (int i = 0; i < numBcastShimColumns; i++)
            vLocs.push_back(XAie_TileLoc(shimColumn1 + i, 0)); // Insert in order of broadcast direction: shimColumn1->shimColumn2
        ...
        driverStatus |= XAie_PerfCounterControlSet(config_manager::s_pDevInst, tileLoc2, XAIE_PL_MOD, (u8)counterId2, COMMON_XAIETILE_EVENT_SHIM_BROADCAST_A[broadcastId], COMMON_XAIETILE_EVENT_SHIM_PORT_RUNNING[eventPortId2]); //see Table 6-17
        debugMsg(static_cast<std::stringstream &&>(std::stringstream() << "XAie_PerfCounterControlSet: col " << (int)tileLoc2.Col
        ...
        //block event broadcast for unintended direction, see Figure 6-27 in Spec 1.5. In shim tile, only broadcast switch A connects to shim tile event generation, broadcast switch B is just a pure relay
        {
            ...
            driverStatus |= XAie_EventBroadcastBlockMapDir(config_manager::s_pDevInst, westTileLoc, XAIE_PL_MOD, XAIE_EVENT_SWITCH_A, bcastMask, XAIE_EVENT_BROADCAST_WEST | XAIE_EVENT_BROADCAST_NORTH | XAIE_EVENT_BROADCAST_SOUTH); // block west, north & south
            driverStatus |= XAie_EventBroadcastBlockMapDir(config_manager::s_pDevInst, westTileLoc, XAIE_PL_MOD, XAIE_EVENT_SWITCH_B, bcastMask, XAIE_EVENT_BROADCAST_NORTH | XAIE_EVENT_BROADCAST_SOUTH); // block north & south
            ...
            for (int i = 1; i < numBcastShimColumns - 1; i++)
            {
                ...
                driverStatus |= XAie_EventBroadcastBlockMapDir(config_manager::s_pDevInst, intermediateTileLoc, XAIE_PL_MOD, XAIE_EVENT_SWITCH_A, bcastMask, XAIE_EVENT_BROADCAST_NORTH | XAIE_EVENT_BROADCAST_SOUTH); // block north & south
                driverStatus |= XAie_EventBroadcastBlockMapDir(config_manager::s_pDevInst, intermediateTileLoc, XAIE_PL_MOD, XAIE_EVENT_SWITCH_B, bcastMask, XAIE_EVENT_BROADCAST_NORTH | XAIE_EVENT_BROADCAST_SOUTH); // block north & south
            }

            //east shim tile switch A
            XAie_LocType eastTileLoc = XAie_TileLoc(eastShimColumn, 0);
            driverStatus |= XAie_EventBroadcastBlockMapDir(config_manager::s_pDevInst, eastTileLoc, XAIE_PL_MOD, XAIE_EVENT_SWITCH_A, bcastMask, XAIE_EVENT_BROADCAST_EAST | XAIE_EVENT_BROADCAST_NORTH | XAIE_EVENT_BROADCAST_SOUTH); // block east, north & south
        }

        driverStatus |= XAie_EventBroadcast(config_manager::s_pDevInst, tileLoc1, XAIE_PL_MOD, (u8)broadcastId, XAIE_EVENT_USER_EVENT_0_PL);
        driverStatus |= XAie_EventGenerate(config_manager::s_pDevInst, tileLoc1, XAIE_PL_MOD, XAIE_EVENT_USER_EVENT_0_PL);
...
uint64_t profiling::read(std::vector<std::shared_ptr<xaiefal::XAieRsc>>& acquiredResources, bool startTimeDifference)
{
    uint64_t value = 0;
    int driverStatus = AieRC::XAIE_OK; //0
    ...
    if (startTimeDifference)
    {
        ...
        if (pPerfCounterRsc1 && pPerfCounterRsc2)
        {
            ...
            driverStatus |= XAie_PerfCounterGet(config_manager::s_pDevInst, loc1, XAieModuleType1, (u8)id1, &value1);
            ...
            driverStatus |= XAie_PerfCounterGet(config_manager::s_pDevInst, loc2, XAieModuleType2, (u8)id2, &value2);
    ...
    else
    {
        if (auto pPerfCounterRsc = dynamic_cast<xaiefal::XAiePerfCounter*>(acquiredResources[0].get()))
        {
            ...
            driverStatus |= XAie_PerfCounterGet(config_manager::s_pDevInst, loc, XAieModuleType, (u8)id, (u32*)(&value));
...
err_code profiling::stop(std::vector<std::shared_ptr<xaiefal::XAieRsc>>& acquiredResources, bool startTimeDifference)
{
    ...
    for (int i = 0; i < acquiredResources.size(); i++)
    {
        ...
        if (auto pSingleTileRsc = dynamic_cast<xaiefal::XAieSingleTileRsc*>(acquiredResource.get()))
        {
            ...
            {
                driverStatus |= XAie_PerfCounterReset(config_manager::s_pDevInst, loc, XAieModuleType, (u8)id);
                debugMsg(static_cast<std::stringstream &&>(std::stringstream() << "XAie_PerfCounterReset: col " << (int)loc.Col <<
                    " row " << (int)loc.Row << ", module " << XAieModuleType << ", counterId " << id).str());

                driverStatus |= XAie_PerfCounterResetControlReset(config_manager::s_pDevInst, loc, XAieModuleType, (u8)id);
            ...
            {
                driverStatus |= XAie_EventSelectStrmPortReset(config_manager::s_pDevInst, loc, (u8)id);
            ...
            {
                driverStatus |= XAie_EventPCReset(config_manager::s_pDevInst, loc, id);
        ...
        else if (auto pBroadcastRsc = dynamic_cast<xaiefal::XAieBroadcast*>(acquiredResource.get()))
        {
            ...
            if (startTimeDifference && startModule == XAIE_PL_MOD && endModule == XAIE_PL_MOD)
            {
                ...
                driverStatus |= XAie_EventBroadcastReset(config_manager::s_pDevInst, XAie_TileLoc(shimColumn1, 0), XAIE_PL_MOD, broadcastId);

                XAie_LocType westTileLoc = XAie_TileLoc(westShimColumn, 0);

                //west shim tile switch A
                driverStatus |= XAie_EventBroadcastUnblockDir(config_manager::s_pDevInst, westTileLoc, XAIE_PL_MOD, XAIE_EVENT_SWITCH_A, (u8)broadcastId, XAIE_EVENT_BROADCAST_WEST | XAIE_EVENT_BROADCAST_NORTH | XAIE_EVENT_BROADCAST_SOUTH); // unblock west, north & south

                //west shim tile switch B
                driverStatus |= XAie_EventBroadcastUnblockDir(config_manager::s_pDevInst, westTileLoc, XAIE_PL_MOD, XAIE_EVENT_SWITCH_B, (u8)broadcastId, XAIE_EVENT_BROADCAST_NORTH | XAIE_EVENT_BROADCAST_SOUTH); // unblock north & south
                ...
                for (int i = 1; i < numBcastShimColumns - 1; i++)
                {
                    ...
                    //intermediate shim tile switch A
                    driverStatus |= XAie_EventBroadcastUnblockDir(config_manager::s_pDevInst, intermediateTileLoc, XAIE_PL_MOD, XAIE_EVENT_SWITCH_A, (u8)broadcastId, XAIE_EVENT_BROADCAST_NORTH | XAIE_EVENT_BROADCAST_SOUTH); // unblock north & south

                    //intermediate shim tile switch B
                    driverStatus |= XAie_EventBroadcastUnblockDir(config_manager::s_pDevInst, intermediateTileLoc, XAIE_PL_MOD, XAIE_EVENT_SWITCH_B, (u8)broadcastId, XAIE_EVENT_BROADCAST_NORTH | XAIE_EVENT_BROADCAST_SOUTH); // unblock north & south
                }
