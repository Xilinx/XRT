...
TestSCVersion::run(std::shared_ptr<xrt_core::device> dev)
{
  ...
  if (!exp_sc_ver.empty() && sc_ver.compare(exp_sc_ver) != 0) {
    logger(ptree, "Warning", "SC firmware mismatch");
    logger(ptree, "Warning", boost::str(boost::format("SC firmware version %s is running on the platform, but SC firmware version %s is expected for the installed base platform. %s, and %s.")
