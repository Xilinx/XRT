                               ...
                               std::ostream & _output) const
{
  ...
  auto used_watts = _pt.get<std::string>("electrical.power_consumption_watts", "N/A");
  auto power_warn = _pt.get<std::string>("electrical.power_consumption_warning", "N/A");
  _output << m_nagiosFormat % "Max Power" % max_watts % "Watts";
  _output << m_nagiosFormat % "Power" % used_watts % "Watts";
  _output << m_nagiosFormat % "Power Warning" % power_warn % "";
  ...
  for(auto& kv : electricals) {
    ...
    if(volts_is_present) {
      auto voltage_name = boost::format("%s_voltage") % name;
      _output << m_nagiosFormat % voltage_name % pt_sensor.get<std::string>("voltage.volts") % "V";
    }
    if(amps_is_present) {
      auto amp_name = boost::format("%s_amp") % name;
