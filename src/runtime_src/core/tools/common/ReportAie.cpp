                                  ...
                                  std::iostream & _output) const
{
  ...
  try {
    ...
    for (auto& rtp_node : _pt.get_child("aie_metadata.rtps")) {
      ...
      _output << fmtCommon("%d") % "Selector Col" % rtp_node.second.get<uint16_t>("selector_column");
      _output << fmtCommon("%d") % "Selector Lock Id" % rtp_node.second.get<uint16_t>("selector_lock_id");
      _output << fmtCommon("%d") % "Selector Addr" %  rtp_node.second.get<uint64_t>("selector_address");
      _output << fmtCommon("%d") % "Ping Buffer Row" % rtp_node.second.get<uint16_t>("ping_buffer_row");
      _output << fmtCommon("%d") % "Ping Buffer Col" % rtp_node.second.get<uint16_t>("ping_buffer_column");
      _output << fmtCommon("%d") % "Ping Buffer Lock Id" % rtp_node.second.get<uint16_t>("ping_buffer_lock_id");
      _output << fmtCommon("%d") % "Ping Buffer Addr" % rtp_node.second.get<uint64_t>("ping_buffer_address");
      _output << fmtCommon("%d") % "Pong Buffer Row" % rtp_node.second.get<uint16_t>("pong_buffer_row");
      _output << fmtCommon("%d") % "Pong Buffer Col" % rtp_node.second.get<uint16_t>("pong_buffer_column");
      _output << fmtCommon("%d") % "Pong Buffer Lock Id" % rtp_node.second.get<uint16_t>("pong_buffer_lock_id");
      _output << fmtCommon("%d") % "Pong Buffer Addr" % rtp_node.second.get<uint64_t>("pong_buffer_address");
    ...
    for (auto& gmio_node : _pt.get_child("aie_metadata.gmios")) {
      ...
      _output << fmtCommon("%d") % "Shim_col" % gmio_node.second.get<uint16_t>("shim_column");
