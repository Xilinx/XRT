                               ...
                               std::ostream & _output) const
{
  ...
  for(auto& kv : fans) {
    ...
    _output << boost::format("      %-22s: %s %s\n") % "Critical Trigger Temp" %
		pt_fan.get<std::string>("critical_trigger_temp") %
		pt_fan.get<std::string>("critical_trigger_temp_units");
    _output << boost::format("      %-22s: %s %s\n") % "Speed" %
		pt_fan.get<std::string>("fan_speed") % pt_fan.get<std::string>("fan_speed_units");
  }
  _output << std::endl;
}
