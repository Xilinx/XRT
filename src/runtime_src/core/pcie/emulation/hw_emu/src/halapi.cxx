...
{
  xcl_hwemu_hal::hwemu_shim *drv = xcl_hwemu_hal::hwemu_shim::handleCheck(handle);
  if (!drv)
    return -1;
...
unsigned xclProbe()
{
  ...
  for(auto &it:devicesInfo)
  {
    ...
    xcl_hwemu_hal::hwemu_shim *handle = new xcl_hwemu_hal::hwemu_shim(deviceIndex, info, DDRBankList, bUnified, bXPR, fRomHeader, platformData);
    xcl_hwemu_hal::devices[deviceIndex++] = handle;
  }
...
xclDeviceHandle xclOpen(unsigned deviceIndex, const char *logfileName, xclVerbosityLevel level)
{
  ...
  xcl_hwemu_hal::hwemu_shim *handle = NULL;

  bool bDefaultDevice = false;
  std::map<unsigned int, xcl_hwemu_hal::hwemu_shim*>::iterator it = xcl_hwemu_hal::devices.find(deviceIndex);
  if(it != xcl_hwemu_hal::devices.end())
  {
    handle = (*it).second;
  }
  else
  {
    handle = new xcl_hwemu_hal::hwemu_shim(deviceIndex, info, DDRBankList, false, false, fRomHeader, platformData);
    bDefaultDevice = true;
  }

  if (!xcl_hwemu_hal::hwemu_shim::handleCheck(handle)) {
    delete handle;
    handle = 0;
  }
  if(handle)
...
void xclClose(xclDeviceHandle handle)
{
  ...
  try {
    drv->xclClose();
    if (xcl_hwemu_hal::hwemu_shim::handleCheck(handle) && xcl_hwemu_hal::devices.size() == 0)
      delete ((xcl_hwemu_hal::hwemu_shim *)handle);
  }
...
int xclLogMsg(xclDeviceHandle handle, xrtLogMsgLevel level, const char* tag, const char* format, ...)
{
  ...
  int ret = xcl_hwemu_hal::hwemu_shim::xclLogMsg(level, tag, format, args);
  va_end(args);
  return ret;
}
