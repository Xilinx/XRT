...
namespace xcl_hwemu_hal {
  ...
  void MBScheduler::cu_continue(struct xocl_cu *xcu)
  {
    ...
    mParent->xclWrite(XCL_ADDR_KERNEL_CTRL, xcu->base + xcu->addr, (void*)&hwemu_shim::CONTROL_AP_CONTINUE,4);

    // in ert_poll mode acknowlegde done to ERT
    if (xcu->polladdr && xcu->run_cnt) {
      if(ert_version>=30) {
    	  mParent->xclCopyBufferHost2Device(xcu->base + xcu->polladdr, (void*)&hwemu_shim::CONTROL_AP_CONTINUE,4,0,XCL_ADDR_SPACE_DEVICE_RAM);
      } else {
    	  mParent->xclWrite(XCL_ADDR_KERNEL_CTRL,xcu->base + xcu->polladdr, (void*)&hwemu_shim::CONTROL_AP_CONTINUE,4);
      }
    }
  }
  ...
  {
    if ((xcu->ctrlreg & hwemu_shim::CONTROL_AP_START) || (!xcu->dataflow && xcu->run_cnt))
      cu_poll(xcu);

    bool bReady = xcu->dataflow ? !(xcu->ctrlreg & hwemu_shim::CONTROL_AP_START) : xcu->run_cnt == 0;
    return bReady;
  }
  ...
  bool MBScheduler::cu_start(struct xocl_cu *xcu, struct xocl_cmd *xcmd)
  {
    ...
    // to next ready check.
    xcu->ctrlreg |= hwemu_shim::CONTROL_AP_START;
    mParent->xclWrite(XCL_ADDR_KERNEL_CTRL, xcu->base + xcu->addr,(void*)& hwemu_shim::CONTROL_AP_START, 4);

    // in ert poll mode request ERT to poll CU
    if (xcu->polladdr) {
      if(ert_version>=30) {
          mParent->xclCopyBufferHost2Device(xcu->base + xcu->polladdr, (void*)&hwemu_shim::CONTROL_AP_START,4,0,XCL_ADDR_SPACE_DEVICE_RAM);
      } else {
    	  mParent->xclWrite(XCL_ADDR_KERNEL_CTRL, xcu->base + xcu->polladdr, (void*)& hwemu_shim::CONTROL_AP_START, 4);
      }
    }
  ...
  void cu_reset(xocl_cu* xcu, unsigned int idx, uint32_t base, uint32_t addr, uint64_t polladdr)
  {
    ...
    xcu->ap_check = (xcu->dataflow) ? (hwemu_shim::CONTROL_AP_DONE ) : (hwemu_shim::CONTROL_AP_DONE | hwemu_shim::CONTROL_AP_IDLE);
  ...
  MBScheduler::MBScheduler(hwemu_shim* _parent)
  {
    mParent = _parent;
