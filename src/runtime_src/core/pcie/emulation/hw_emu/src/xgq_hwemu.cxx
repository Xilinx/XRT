...
using namespace xcl_hwemu_hal;

namespace hwemu {

  void xgq_hwemu_mem_write32(uint64_t io_hdl, uint64_t addr, uint32_t val)
  {
    auto device = reinterpret_cast<xcl_hwemu_hal::hwemu_shim *>(io_hdl);
  ...
  xgq_queue::xgq_queue(hwemu_shim* in_dev, xocl_xgq* in_xgqp, uint16_t in_nslot, uint32_t in_slot_size, uint64_t in_xgq_sub_base, uint64_t in_xgq_com_base)
    : device(in_dev)
    , xgqp(in_xgqp)
    , nslot(in_nslot)
  ...
  xocl_xgq::xocl_xgq(hwemu_shim* dev)
