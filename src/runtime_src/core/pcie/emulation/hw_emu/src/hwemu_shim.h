...
#ifndef _HWEMU_SHIM_H_
#define _HWEMU_SHIM_H_
 ...
 typedef struct
 {
  ...
  class hwemu_shim {
    ...
    public:
      ...
      // Sanity checks
      static hwemu_shim *handleCheck(void *handle);
      uint32_t getAddressSpace (uint32_t topology);

      //constructor
      hwemu_shim( unsigned int deviceIndex, xclDeviceInfo2 &info, std::list<xclemulation::DDRBank>& DDRBankList, bool bUnified,
  ...
  extern std::map<unsigned int, hwemu_shim*> devices;
 }
#endif
