...
namespace xcl_hwemu_hal {
    ...
    class Q2H_helper {
        ...
        xcl_hwemu_hal::hwemu_shim* inst;

        public:
        Q2H_helper(xcl_hwemu_hal::hwemu_shim* _inst);
        ~Q2H_helper();
        int  poolingon_Qdma();
        bool connect_sock();
    };
  ...
  std::map<unsigned int, hwemu_shim*> devices;
  std::map<std::string, std::string> hwemu_shim::mEnvironmentNameValueMap(xclemulation::getEnvironmentByReadingIni());
  std::map<int, std::tuple<std::string,int,void*, unsigned int> > hwemu_shim::mFdToFileNameMap;
  std::ofstream hwemu_shim::mDebugLogStream;
  bool hwemu_shim::mFirstBinary = true;
  unsigned int hwemu_shim::mBufferCount = 0;
  const int xcl_hwemu_hal::hwemu_shim::SPIR_ADDRSPACE_PRIVATE   = 0;
  const int xcl_hwemu_hal::hwemu_shim::SPIR_ADDRSPACE_GLOBAL    = 1;
  const int xcl_hwemu_hal::hwemu_shim::SPIR_ADDRSPACE_CONSTANT  = 2;
  const int xcl_hwemu_hal::hwemu_shim::SPIR_ADDRSPACE_LOCAL     = 3;
  const int xcl_hwemu_hal::hwemu_shim::SPIR_ADDRSPACE_PIPES     = 4;
  const unsigned hwemu_shim::CONTROL_AP_START = 1;
  const unsigned hwemu_shim::CONTROL_AP_DONE  = 2;
  const unsigned hwemu_shim::CONTROL_AP_IDLE  = 4;
  const unsigned hwemu_shim::CONTROL_AP_CONTINUE  = 0x10;
  const unsigned hwemu_shim::REG_BUFF_SIZE = 0x4;
  const unsigned hwemu_shim::M2M_KERNEL_ARGS_SIZE = 36;
  ...
  size_t hwemu_shim::alloc_void(size_t new_size)
  {
    if (buf_size == 0)
    {
      buf = malloc(new_size);
  ...
  {
    std::map<unsigned int, hwemu_shim*>::iterator start = devices.begin();
    std::map<unsigned int, hwemu_shim*>::iterator end = devices.end();
    for(; start != end; start++)
    {
      hwemu_shim* handle = (*start).second;
      if(!handle)
        continue;
  ...
  std::string hwemu_shim::loadFileContentsToString(const std::string& path)
  {
    std::ifstream file(path);
    return {std::istreambuf_iterator<char>(file), std::istreambuf_iterator<char>()};
  }

  int hwemu_shim::parseLog()
  {
  ...
  void hwemu_shim::parseString(const std::string& simPath , const std::string& searchString)
  {
  ...
  void hwemu_shim::parseSimulateLog ()
  {
  ...
  bool hwemu_shim::isUltraScale() const
  {
    return false;
  }

  int hwemu_shim::xclLoadXclBin(const xclBin *header)
  {
    ...
    {
      hwemu_shim::mDebugLogStream.open(xclemulation::getEmDebugLogFile(),std::ofstream::out);
  ...
  int hwemu_shim::xclLoadBitstreamWorker(bitStreamArg args)
  {
  ...
  bool hwemu_shim::readEmuSettingsJsonFile(const std::string& emuSettingsFilePath) {
  ...
  void hwemu_shim::createPreSimScript(const std::string& wcfgFilePath, std::string& preSimScriptPath) {
  ...
  void hwemu_shim::extractEmuData(const std::string& simPath, int binaryCounter, bitStreamArg args) {
   ...
   size_t hwemu_shim::xclWrite(xclAddressSpace space, uint64_t offset, const void *hostBuf, size_t size) {
  ...
  size_t hwemu_shim::xclRead(xclAddressSpace space, uint64_t offset, void *hostBuf, size_t size) {
...
uint32_t hwemu_shim::getAddressSpace (uint32_t topology)
{
  ...
  size_t hwemu_shim::xclCopyBufferHost2Device(uint64_t dest, const void *src, size_t size, size_t seek, uint32_t topology)
  {
    if(!sock)
    {
      if(!mMemModel)
  ...
  size_t hwemu_shim::xclCopyBufferDevice2Host(void *dest, uint64_t src, size_t size, size_t skip, uint32_t topology)
  {
  ...
  uint64_t hwemu_shim::xclAllocDeviceBuffer(size_t size)
  {
    if (mLogStream.is_open()) {
  ...
  uint64_t hwemu_shim::xclAllocDeviceBuffer2(size_t& size, xclMemoryDomains domain, unsigned flags, bool noHostMemory, unsigned boFlags, std::string &sFileName, std::map<uint64_t,uint64_t>& chunks)
  {
    if (mLogStream.is_open()) {
  ...
  void hwemu_shim::xclFreeDeviceBuffer(uint64_t offset, bool sendtoxsim)
  {
    if (mLogStream.is_open()) {
  ...
  void hwemu_shim::logMessage(std::string& msg , int verbosity)
  {
  ...
  void hwemu_shim::saveWaveDataBase()
  {
    if (mLogStream.is_open()) {
      mLogStream << __func__ << ", " << std::this_thread::get_id() << std::endl;
    }
  ...
  int hwemu_shim::resetProgram(bool saveWdb)
  {
    if (mLogStream.is_open()) {
      mLogStream << __func__ << ", " << std::this_thread::get_id() << std::endl;
    }
  ...
  hwemu_shim *hwemu_shim::handleCheck(void *handle) {
    if (!handle)
      return 0;

    return (hwemu_shim *)handle;

  }

  hwemu_shim::~hwemu_shim() {
    free(ci_buf);
    free(ri_buf);
    free(buf);
    parsedMsgs.clear();
  ...
  void hwemu_shim::initMemoryManager(std::list<xclemulation::DDRBank>& DDRBankList)
  {
  ...
  std::string hwemu_shim::getSimulatorType(const std::string& binaryDirectory) {
  ...
  void hwemu_shim::fillDeviceInfo(xclDeviceInfo2* dest, xclDeviceInfo2* src)
  {
  ...
  hwemu_shim::hwemu_shim(unsigned int deviceIndex, xclDeviceInfo2 &info, std::list<xclemulation::DDRBank>& DDRBankList, bool _unified, bool _xpr,
  ...
  bool hwemu_shim::isMBSchedulerEnabled()
  {
  ...
  void hwemu_shim::constructQueryTable() {
    if (xclemulation::config::getInstance()->getIsPlatformEnabled()) {
  ...
  uint64_t hwemu_shim::getM2MAddress() {
    if (xclemulation::config::getInstance()->getIsPlatformEnabled()) {
  ...
  uint64_t hwemu_shim::getErtCmdQAddress() {
  ...
  uint64_t hwemu_shim::getErtBaseAddress() {
    if (xclemulation::config::getInstance()->getIsPlatformEnabled()) {
  ...
  std::shared_ptr<xrt_core::device> hwemu_shim::getMCoreDevice()
  {
    return mCoreDevice;
  }

  bool hwemu_shim::isLegacyErt()
  {
  ...
  uint64_t hwemu_shim::getCdmaBaseAddress(unsigned int index)
  {
    if (xclemulation::config::getInstance()->getIsPlatformEnabled()) {
  ...
  unsigned int hwemu_shim::getDsaVersion()
  {
    std::string vbnv  = mDeviceInfo.mName;
    if(vbnv.empty())
      return 52;
  ...
  size_t hwemu_shim::xclGetDeviceTimestamp()
  {
    bool ack = true;
  ...
  void hwemu_shim::xclReadBusStatus(xclPerfMonType type) {
  ...
  void hwemu_shim::xclGetDebugMessages(bool force)
  {
  ...
  size_t hwemu_shim::xclReadSkipCopy(uint64_t offset, void *hostBuf, size_t size)
  {
    if (mLogStream.is_open()) {
  ...
  size_t hwemu_shim::xclReadModifyWrite(uint64_t offset, const void *hostBuf, size_t size)
  {
    if (mLogStream.is_open()) {
      mLogStream << __func__ << ", " << std::this_thread::get_id() << ", "
        << offset << ", " << hostBuf << ", " << size << std::endl;
  ...
  int hwemu_shim::xclGetDeviceInfo2(xclDeviceInfo2 *info)
  {
  ...
  void hwemu_shim::xclOpen(const char* logfileName)
  {
...
static bool check_bo_user_flags(hwemu_shim* dev, unsigned flags)
{
...
xclemulation::drm_xocl_bo* hwemu_shim::xclGetBoByHandle(unsigned int boHandle)
{
...
inline unsigned short hwemu_shim::xocl_ddr_channel_count()
{
  if(mMembanks.size() > 0)
...
inline unsigned long long hwemu_shim::xocl_ddr_channel_size()
{
  return 0;
}

int hwemu_shim::xclGetBOProperties(unsigned int boHandle, xclBOProperties *properties)
{
...
/******************************** xclAllocBO *********************************************/
uint64_t hwemu_shim::xoclCreateBo(xclemulation::xocl_create_bo* info)
{
...
unsigned int hwemu_shim::xclAllocBO(size_t size, int unused, unsigned flags)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
  {
...
/******************************** xclAllocUserPtrBO ************************************/
unsigned int hwemu_shim::xclAllocUserPtrBO(void *userptr, size_t size, unsigned flags)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
  {
...
/******************************** xclExportBO *******************************************/
int hwemu_shim::xclExportBO(unsigned int boHandle)
{
  //TODO
  if (mLogStream.is_open())
  {
...
/******************************** xclImportBO *******************************************/
unsigned int hwemu_shim::xclImportBO(int boGlobalHandle, unsigned flags)
{
  //TODO
  if (mLogStream.is_open())
  {
...
/******************************** xclCopyBO *******************************************/
int hwemu_shim::xclCopyBO(unsigned int dst_boHandle, unsigned int src_boHandle, size_t size, size_t dst_offset, size_t src_offset)
{
   std::lock_guard<std::mutex> lk(mApiMtx);
  //TODO
  if (mLogStream.is_open())
...
/******************************** xclMapBO *********************************************/
void *hwemu_shim::xclMapBO(unsigned int boHandle, bool write)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
  {
...
int hwemu_shim::xclUnmapBO(unsigned int boHandle, void* addr)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
...
/******************************** xclSyncBO *******************************************/
int hwemu_shim::xclSyncBO(unsigned int boHandle, xclBOSyncDirection dir, size_t size, size_t offset)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
  {
...
/******************************** xclFreeBO *******************************************/
void hwemu_shim::xclFreeBO(unsigned int boHandle)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
  {
...
/******************************** xclWriteBO *******************************************/
size_t hwemu_shim::xclWriteBO(unsigned int boHandle, const void *src, size_t size, size_t seek)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
  {
...
/******************************** xclReadBO *******************************************/
size_t hwemu_shim::xclReadBO(unsigned int boHandle, void *dst, size_t size, size_t skip)
{
  std::lock_guard<std::mutex> lk(mApiMtx);
  if (mLogStream.is_open())
  {
...
int hwemu_shim::xclExecBuf(unsigned int cmdBO)
{
 ...
 * xclExecBuf()
 */
int hwemu_shim::xclExecBuf(unsigned int cmdBO, size_t num_bo_in_wait_list, unsigned int *bo_wait_list)
{
  if (mLogStream.is_open())
  {
...
int hwemu_shim::xclRegisterEventNotify(unsigned int userInterrupt, int fd)
{
  if (mLogStream.is_open())
  {
...
int hwemu_shim::xclExecWait(int timeoutMilliSec)
{
  if (mLogStream.is_open())
  {
...
ssize_t hwemu_shim::xclUnmgdPwrite(unsigned flags, const void *buf, size_t count, uint64_t offset)
{
  if (flags)
    return -EINVAL;
...
ssize_t hwemu_shim::xclUnmgdPread(unsigned flags, void *buf, size_t count, uint64_t offset)
{
  if (flags)
    return -EINVAL;
...
int hwemu_shim::xclGetDebugIPlayoutPath(char* layoutPath, size_t size)
{
...
int hwemu_shim::xclGetTraceBufferInfo(uint32_t nSamples, uint32_t& traceSamples, uint32_t& traceBufSz)
{
...
int hwemu_shim::xclReadTraceData(void* traceBuf, uint32_t traceBufSz, uint32_t numSamples, uint64_t ipBaseAddress, uint32_t& wordsPerSample)
{
...
double hwemu_shim::xclGetDeviceClockFreqMHz()
{
  //return 1.0;
  double clockSpeed;
  //300.0 MHz
...
double hwemu_shim::xclGetHostReadMaxBandwidthMBps()
{
  return 15753.85;
}
...
// For PCIe gen 3x16 or 4x8:
// Max BW = 16.0 * (128b/130b encoding) = 15.75385 GB/s
double hwemu_shim::xclGetHostWriteMaxBandwidthMBps()
{
  return 15753.85;
}

// For DDR4: Typical Max BW = 19.25 GB/s
double hwemu_shim::xclGetKernelReadMaxBandwidthMBps()
{
  return 19250.00;
}

// For DDR4: Typical Max BW = 19.25 GB/s
double hwemu_shim::xclGetKernelWriteMaxBandwidthMBps()
{
  return 19250.00;
}

uint32_t hwemu_shim::getPerfMonNumberSlots(xclPerfMonType type)
{
...
// Get slot name
void hwemu_shim::getPerfMonSlotName(xclPerfMonType type, uint32_t slotnum,
 ...
 * xclCreateWriteQueue()
 */
int hwemu_shim::xclCreateWriteQueue(xclQueueContext *q_ctx, uint64_t *q_hdl)
{
 ...
 * xclCreateReadQueue()
 */
int hwemu_shim::xclCreateReadQueue(xclQueueContext *q_ctx, uint64_t *q_hdl)
{
  if (mLogStream.is_open())
  {
    mLogStream << __func__ << ", " << std::this_thread::get_id() << std::endl;
 ...
 * xclDestroyQueue()
 */
int hwemu_shim::xclDestroyQueue(uint64_t q_hdl)
{
  if (mLogStream.is_open())
  {
    mLogStream << __func__ << ", " << std::this_thread::get_id() << std::endl;
 ...
 * xclWriteQueue()
 */
ssize_t hwemu_shim::xclWriteQueue(uint64_t q_hdl, xclQueueRequest *wr)
{
 ...
 * xclReadQueue()
 */
ssize_t hwemu_shim::xclReadQueue(uint64_t q_hdl, xclQueueRequest *rd)
{
  if (mLogStream.is_open())
  {
    mLogStream << __func__ << ", " << std::this_thread::get_id() << std::endl;
 ...
 * xclPollCompletion
 */
int hwemu_shim::xclPollCompletion(int min_compl, int max_compl, xclReqCompletion *comps, int* actual, int timeout)
{
  if (mLogStream.is_open())
  {
 ...
 * xclAllocQDMABuf()
 */
void * hwemu_shim::xclAllocQDMABuf(size_t size, uint64_t *buf_hdl)
{
  if (mLogStream.is_open())
  {
    mLogStream << __func__ << ", " << std::this_thread::get_id() << std::endl;
 ...
 * xclFreeQDMABuf()
 */
int hwemu_shim::xclFreeQDMABuf(uint64_t buf_hdl)
{
  if (mLogStream.is_open())
  {
    mLogStream << __func__ << ", " << std::this_thread::get_id() << std::endl;
...
int hwemu_shim::xcl_LogMsg(xrtLogMsgLevel level, const char* tag, const char* format, ...)
{
  va_list args;
 ...
 * xclLogMsg()
 */
int hwemu_shim::xclLogMsg(xrtLogMsgLevel level, const char* tag, const char* format, va_list args1)
{
...
void hwemu_shim::closeMessengerThread()
{
...
int hwemu_shim::xclRegRW(bool rd, uint32_t cu_index, uint32_t offset, uint32_t *datap)
{
  if (mLogStream.is_open()) {
...
int hwemu_shim::xclRegRead(uint32_t cu_index, uint32_t offset, uint32_t *datap)
{
  return xclRegRW(true, cu_index, offset, datap);
}

int hwemu_shim::xclRegWrite(uint32_t cu_index, uint32_t offset, uint32_t data)
{
  return xclRegRW(false, cu_index, offset, &data);
}

//Get CU index from IP_LAYOUT section for corresponding kernel name
int hwemu_shim::xclIPName2Index(const char *name)
{
 ...
 *
 **/
bool hwemu_shim::device2xrt_rd_trans_cb(unsigned long int addr, void* const data_ptr,unsigned long int size) {
 ...
 *
 **/
bool hwemu_shim::device2xrt_wr_trans_cb(unsigned long int addr, void const* data_ptr,unsigned long int size) {
...
bool hwemu_shim::device2xrt_irq_trans_cb(uint32_t,unsigned long int) {
    // TODO: We need to return a ERROR here as we are not supporting this. Its helps users to get notified
    return true;
}
Q2H_helper :: Q2H_helper(xcl_hwemu_hal::hwemu_shim* _inst) {
...
void hwemu_shim::hostMemAccessThread() {
