 ...
 * under the License.
 */
#ifndef _SWEMU_SHIM_H_
#define _SWEMU_SHIM_H_
...
namespace xcl_swemu_hal {
  ...
  public:
      ...
      ~swemu_shim();
      swemu_shim(unsigned int deviceIndex, xclDeviceInfo2 &info, std::list<xclemulation::DDRBank>& DDRBankList, bool bUnified,
        bool bXPR, FeatureRomHeader &featureRom, const boost::property_tree::ptree & platformData);

      static swemu_shim *handleCheck(void *handle);
      bool isGood() const;
  ...
  class GraphType {
    ...
    // be extracted
  public:
    GraphType(xcl_swemu_hal::swemu_shim* handle, const char* graph) {
    ...
    xcl_swemu_hal::swemu_shim*  getDeviceHandle() {  return _deviceHandle;  }
    const char*  getGraphName() { return _graph; }
    unsigned int  getGraphHandle() { return graphHandle; }
  private:
    xcl_swemu_hal::swemu_shim*  _deviceHandle;
  ...
  extern std::map<unsigned int, swemu_shim*> devices;
  ...
  struct sParseLog
  {
    ...
    swemu_shim * mCpuShimPtr;

    sParseLog(swemu_shim * iPtr, const std::string& iDeviceLog)
