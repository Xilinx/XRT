...
xclDeviceHandle xclOpen(unsigned deviceIndex, const char *logfileName, xclVerbosityLevel level)
{
  ...
  xcl_swemu_hal::swemu_shim *handle = NULL;
  bool bDefaultDevice = false;
  std::map<unsigned int, xcl_swemu_hal::swemu_shim*>::iterator it = xcl_swemu_hal::devices.find(deviceIndex);
  if(it != xcl_swemu_hal::devices.end())
  {
    handle = (*it).second;
  }
  else
  {
    handle = new xcl_swemu_hal::swemu_shim(deviceIndex, info, DDRBankList, false, false, fRomHeader, platformData);
    bDefaultDevice = true;
  }

  if (!xcl_swemu_hal::swemu_shim::handleCheck(handle)) {
    delete handle;
    handle = 0;
  }
  if (handle) {
...
{
  xcl_swemu_hal::swemu_shim *drv = xcl_swemu_hal::swemu_shim::handleCheck(handle);
  if (!drv)
    return ;
  drv->xclClose();
  if (xcl_swemu_hal::swemu_shim::handleCheck(handle) && xcl_swemu_hal::devices.size() == 0) {
    delete ((xcl_swemu_hal::swemu_shim*)handle);
  }
}
...
unsigned xclProbe()
{
  ...
  for(auto &it: devicesInfo)
  {
    ...
    xcl_swemu_hal::swemu_shim *handle = new xcl_swemu_hal::swemu_shim(deviceIndex, info, DDRBankList, bUnified, bXPR, fRomHeader, platformData);
    xcl_swemu_hal::devices[deviceIndex++] = handle;
  }
...
int xclLogMsg(xclDeviceHandle handle, xrtLogMsgLevel level, const char* tag, const char* format, ...)
{
  ...
  int ret = xcl_swemu_hal::swemu_shim::xclLogMsg(handle, level, tag, format, args);
  va_end(args);
  return ret;
}
...
xclGraphOpen(xclDeviceHandle handle, const uuid_t xclbin_uuid, const char* graph, xrt::graph::access_mode am)
{
  try {
    xcl_swemu_hal::swemu_shim *drv = xcl_swemu_hal::swemu_shim::handleCheck(handle);
    xclGraphHandle graphHandle = new xcl_swemu_hal::GraphType(drv, graph);
    if (graphHandle) {
      auto ghPtr = (xcl_swemu_hal::GraphType*)graphHandle;
...
xclGraphClose(xclGraphHandle ghl)
{
  try {
    if (ghl) {
      auto ghPtr = (xcl_swemu_hal::GraphType*)ghl;
      delete ghPtr;
    }
  }
  catch (const std::exception& ex) {
...
xclGraphUpdateRTP(xclGraphHandle ghdl, const char* port, const char* buffer, size_t size)
{
  try {
    if (ghdl) {
      auto ghPtr = (xcl_swemu_hal::GraphType*)ghdl;
      auto drv = (ghPtr) ? ghPtr->getDeviceHandle() : nullptr;
