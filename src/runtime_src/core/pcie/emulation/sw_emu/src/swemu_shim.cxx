...
namespace xcl_swemu_hal {
  ...
  std::map<int, std::tuple<std::string, uint64_t, void*> > swemu_shim::mFdToFileNameMap;
...
#define PRINTENDFUNC if (mLogStream.is_open()) mLogStream << __func__ << " ended " << std::endl;
  ...
  swemu_shim::swemu_shim(unsigned int deviceIndex, xclDeviceInfo2 &info, std::list<xclemulation::DDRBank>& DDRBankList, bool _unified, bool _xpr,
...
//private
  ...
  std::string swemu_shim::getDeviceProcessLogPath() 
  {
  ...
  uint64_t swemu_shim::xclAllocDeviceBuffer2(size_t& size, xclMemoryDomains domain, unsigned flags, bool zeroCopy, std::string &sFileName)
  {
    if (mLogStream.is_open()) {
  ...
  void swemu_shim::closeMessengerThread()
  {
    if (mMessengerThread.joinable())
      mMessengerThread.join();
  }

  void swemu_shim::messagesThread()
  {
...
int swemu_shim::xclOpenContextByName(uint32_t slot, const uuid_t xclbinId, const char* cuname, bool shared) const
{
...
void swemu_shim::constructQueryTable() {
...
int swemu_shim::deviceQuery(key_type queryKey) {
