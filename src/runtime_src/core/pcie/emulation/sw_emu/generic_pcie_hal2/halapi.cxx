...
xclDeviceHandle xclOpen(unsigned deviceIndex, const char *logfileName, xclVerbosityLevel level)
{
  ...
  if (handle) {
    ...
    if (bDefaultDevice)
    {
      std::string sDummyDeviceMsg = "CRITICAL WARNING: [SW-EMU 09-0] Unable to find emconfig.json. Using default device \"xilinx:pcie-hw-em:7v3:1.0\"";
...
unsigned xclProbe()
{
  ...
  {
    std::string initMsg ="ERROR: [SW-EMU 09] Please set XCL_EMULATION_MODE to \"sw_emu\" to run software emulation. ";
