...
// is required
static xrt_core::aws::system*
singleton_instance()
{
  static xrt_core::aws::system singleton;
  return &singleton;
}
...
namespace xrt_core { namespace aws {
...
get_userpf_device(device::handle_type handle, device::id_type id) const
{
  ...
  return std::shared_ptr<xrt_core::aws::device>(new xrt_core::aws::device(handle, id, true));
}
...
{
  // deliberately not using std::make_shared (used with weak_ptr)
  return std::shared_ptr<xrt_core::aws::device>(new xrt_core::aws::device(nullptr, id, false));
}
