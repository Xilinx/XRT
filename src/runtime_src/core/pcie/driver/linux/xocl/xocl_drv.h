...
#define XOCL_GET_MEM_TOPOLOGY(xdev)						\
	(XOCL_DSA_IS_VERSAL(xdev) ?					\
	((struct mem_topology *)(((struct xocl_dev *)(xdev))->mem_topo)) : \
	((struct mem_topology *)xocl_icap_get_xclbin_metadata(xdev, MEMTOPO_AXLF)))
#define XOCL_GET_IP_LAYOUT(xdev)						\
	((struct ip_layout *)xocl_icap_get_xclbin_metadata(xdev, IPLAYOUT_AXLF))
#define XOCL_GET_XCLBIN_ID(xdev)						\
	((xuid_t *)xocl_icap_get_xclbin_metadata(xdev, XCLBIN_UUID))
...
({ \
	...
	uint32_t ret = topo ? topo->m_count : 0; \
	XOCL_PUT_MEM_TOPOLOGY(xdev); \
	(ret); \
})
...
struct xocl_icap_funcs {
	...
	uint64_t (*get_data)(struct platform_device *pdev,
		enum data_kind kind);
	uint64_t (*get_xclbin_metadata)(struct platform_device *pdev,
		enum data_kind kind);
...
#define	xocl_icap_get_data(xdev, kind)					\
	...
	0)
#define	xocl_icap_get_xclbin_metadata(xdev, kind)			\
	(ICAP_CB(xdev, get_xclbin_metadata) ?				\
	ICAP_OPS(xdev)->get_xclbin_metadata(ICAP_DEV(xdev), kind) : 	\
	0)
