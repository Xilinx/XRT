...
struct xocl_rom_funcs {
	...
	void (*aws_set_vbnv_name)(struct platform_device *pdev);
};
...
#define xocl_rom_aws_set_vbnv_name(xdev)				\
	(ROM_CB(xdev, aws_set_vbnv_name) ?				\
	ROM_OPS(xdev)->aws_set_vbnv_name(ROM_DEV(xdev)) : NULL)
