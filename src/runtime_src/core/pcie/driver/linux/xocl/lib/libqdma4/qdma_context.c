			   ...
			   struct qdma_descq_sw_ctxt *sw_ctxt)
{
	...
	if ((descq->xdev->conf.qdma_drv_mode == INDIRECT_INTR_MODE) ||
		...
		sw_ctxt->vec = qdma4_get_intr_ring_index(descq->xdev,
						       descq->intr_id);
	...
	struct qdma_qid2vec *cntxt)
{
	...
	if ((descq->xdev->conf.qdma_drv_mode == INDIRECT_INTR_MODE) ||
			(descq->xdev->conf.qdma_drv_mode == AUTO_MODE)) {
		vec_num = qdma4_get_intr_ring_index(xdev, descq->intr_id);
		en_coal = 1;
	}
			     ...
			     struct qdma_descq_cmpt_ctxt *cmpt_ctxt)
{
	...
	if ((descq->xdev->conf.qdma_drv_mode == INDIRECT_INTR_MODE) ||
			(descq->xdev->conf.qdma_drv_mode == AUTO_MODE)) {
		ring_index = qdma4_get_intr_ring_index(descq->xdev, descq->intr_id);
...
#ifdef __QDMA_VF__
int qdma4_intr_context_setup(struct xlnx_dma_dev *xdev)
{
	int i = 0;
	int rv;
	...
	for (i = 0; i < QDMA_NUM_DATA_VEC_FOR_INTR_CXT; i++) {
		ictxt.ring_index_list[i] =
			qdma4_get_intr_ring_index(xdev, xdev->dvec_start_idx + i);
	}
...
int qdma4_intr_context_read(struct xlnx_dma_dev *xdev,
				   int ring_index,
				   struct qdma_indirect_intr_ctxt *ctxt)
{
	struct mbox_msg *m;
...
int qdma4_descq_context_clear(struct xlnx_dma_dev *xdev, unsigned int qid_hw,
...
int qdma4_descq_context_read(struct xlnx_dma_dev *xdev, unsigned int qid_hw,
...
int qdma4_descq_context_setup(struct qdma_descq *descq)
{
	...
	if ((descq->xdev->conf.qdma_drv_mode == INDIRECT_INTR_MODE) ||
			(descq->xdev->conf.qdma_drv_mode == AUTO_MODE)) {
		int ring_index = qdma4_get_intr_ring_index(descq->xdev,
						     descq->intr_id);
...
int qdma_descq_context_dump(struct qdma_descq *descq, char *buf, int buflen)
{
	...
	rv = qdma4_descq_context_read(descq->xdev, descq->qidx_hw,
	...
	if ((descq->xdev->conf.qdma_drv_mode == INDIRECT_INTR_MODE) ||
		...
		for (ring_count = 0;
				...
				ring_count++) {
			ring_index = qdma4_get_intr_ring_index(
						descq->xdev,
						(descq->xdev->dvec_start_idx +
								ring_count));

			rv = qdma4_intr_context_read(descq->xdev,
...
int qdma4_prog_intr_context(struct xlnx_dma_dev *xdev,
		struct mbox_msg_intr_ctxt *ictxt)
{
	int i = 0;
	int rv;
...
int qdma4_intr_context_setup(struct xlnx_dma_dev *xdev)
{
	...
	for (i = 0; i <  QDMA_NUM_DATA_VEC_FOR_INTR_CXT; i++) {
		ring_index = qdma4_get_intr_ring_index(xdev,
...
int qdma4_descq_context_setup(struct qdma_descq *descq)
{
	...
	return qdma4_descq_context_program(descq->xdev, descq->qidx_hw,
...
int qdma4_descq_context_program(struct xlnx_dma_dev *xdev, unsigned int qid_hw,
			bool st, u8 type, struct qdma_descq_context *context)
{
	int rv;

	/* always clear first */
	rv = qdma4_descq_context_clear(xdev, qid_hw, st, type, 1);
	if (rv < 0) {
