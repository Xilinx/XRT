...
int qdma4_global_csr_get(unsigned long dev_hndl, u8 index, u8 count,
		struct global_csr_conf *csr)
{
	struct xlnx_dma_dev *xdev = (struct xlnx_dma_dev *)dev_hndl;
	int i = 0;
...
int qdma4_device_flr_quirk_set(struct pci_dev *pdev, unsigned long dev_hndl)
{
	...
	if (!dev_hndl || qdma4_xdev_check_hndl(__func__, pdev, dev_hndl) < 0)
		return -EINVAL;
...
#ifndef __QDMA_VF__
void qdma4_device_attributes_get(struct xlnx_dma_dev *xdev)
{
...
int qdma4_queue_cmpl_ctrl(unsigned long dev_hndl, unsigned long id,
