...
int qdma_device_interrupt_setup(struct xlnx_dma_dev *xdev)
{
	...
	if ((xdev->conf.qdma_drv_mode == INDIRECT_INTR_MODE) ||
		...
		if (xdev->intr_coal_list != NULL) {
			rv = qdma4_intr_context_setup(xdev);
			if (rv)
				return rv;
		} else {
	...
	if (((xdev->conf.qdma_drv_mode != POLL_MODE) &&
			...
			xdev->conf.master_pf) {
		rv = qdma4_err_qdma4_intr_setup(xdev);
		if (rv < 0) {
...
int qdma4_device_init(struct xlnx_dma_dev *xdev)
{
	...
	for (i = 0, descq = qdev->h2c_descq; i < qdev->qmax; i++, descq++)
		qdma4_descq_init(descq, xdev, i, i);
...
void qdma4_device_cleanup(struct xlnx_dma_dev *xdev)
{
	...
	for (i = 0, descq = qdev->h2c_descq; i < qdev->qmax; i++, descq++) {
		if (descq->q_state == Q_STATE_ONLINE)
			qdma4_queue_stop((unsigned long int)xdev,
					i, buf, QDMA_BUF_LEN);
	}
	...
	for (i = 0, descq = qdev->h2c_descq; i < qdev->qmax; i++, descq++) {
		if (descq->q_state == Q_STATE_ENABLED)
			qdma4_queue_remove((unsigned long int)xdev,
					i, buf, QDMA_BUF_LEN);
	}
