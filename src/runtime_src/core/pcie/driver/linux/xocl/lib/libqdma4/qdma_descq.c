...
/* TSC */
	...
	asm volatile("mftbu %0" : "=r" (tbhi));
        ...
        return 1000 * ((uint64_t) tbhi << 32) | tblo;
#endif
#else
	return 0UL;
#endif
...
static int descq_mm_n_h2c_cmpl_status(struct qdma_descq *descq)
{
	...
	qdma4_incr_cmpl_desc_cnt(descq, cr);
			...
			int idx_hw, int idx_sw)
{
	...
	INIT_WORK(&descq->work, qdma4_intr_work);
	INIT_WORK(&descq->req_work, qdma4_c2h_req_work);
	descq->xdev = xdev;
	descq->channel = 0;
...
int qdma4_descq_config_complete(struct qdma_descq *descq)
{
	...
	if ((qconf->st && (qconf->q_type == Q_C2H)) ||
		...
		/* per queue irq needs to be in sync with per device setting */
		if (descq->cmpt_cidx_info.irq_en &&
		    descq->xdev->conf.qdma_drv_mode == POLL_MODE) 
...
{
	int rv = qdma4_descq_context_setup(descq);
...
int qdma4_descq_dump_desc(struct qdma_descq *descq, int start,
...
int qdma4_descq_dump_cmpt(struct qdma_descq *descq, int start,
			int end, char *buf, int buflen)
{
...
int qdma4_queue_avail_desc(unsigned long dev_hndl, unsigned long id)
{
	struct qdma_descq *descq = qdma4_device_get_descq_by_id(
...
int qdma4_queue_packet_write(unsigned long dev_hndl, unsigned long id,
...
int qdma4_descq_get_cmpt_udd(unsigned long dev_hndl, unsigned long id,
		char *buf, int buflen)
{
	uint8_t *cmpt;
	uint8_t i = 0;
...
int qdma4_descq_read_cmpt_data(unsigned long dev_hndl, unsigned long id,
