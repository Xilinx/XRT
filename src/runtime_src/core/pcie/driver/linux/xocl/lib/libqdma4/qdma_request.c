...
{
	pr_info("%s, req 0x%p %u, ep 0x%llx, tm %u ms, %s,%s,%s,%s,%s,async %d.\n",
                str, req, req->count, req->ep_addr, req->timeout_ms,
                req->write ? "W":"R", req->dma_mapped ? "M":"",
                req->h2c_eot ? "EOT":"", req->use_sgt ? "SGT":"SGL",
...
/*****************************************************************************/
/**
 * qdma4_request_unmap() - unmap the request's data buffers
 *
...
/*****************************************************************************/
/**
 * qdma4_request_map() - map the request's data buffers to bus address
 *
 * @param[in]   pdev:   pointer to struct pci_dev
 * @param[in]   req:	request
 *
				...
				struct qdma_request *req)
{
	...
	if (cb->unmap_needed) {
	       qdma4_request_unmap(descq->xdev->conf.pdev, req);
	       cb->unmap_needed = 0;
	}
			...
			struct qdma_request *req, unsigned int count)
{
	...
	struct qdma_descq *descq =
		qdma4_device_get_descq_by_id(xdev, qhndl, NULL, 0, 0);
	int i;
	unsigned long flags;
