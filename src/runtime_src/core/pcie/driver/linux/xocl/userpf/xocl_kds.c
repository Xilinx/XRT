...
static void xocl_detect_fa_plram(struct xocl_dev *xdev)
{
	...
	bo = __xocl_create_bo_ioctl(XOCL_DRM(xdev)->ddev, &args);
	if (IS_ERR(bo))
		goto done;

	XDEV(xdev)->kds.plram.bar_paddr = page_to_phys(bo->pages[0]);
	XDEV(xdev)->kds.plram.dev_paddr = base_addr;
	XDEV(xdev)->kds.plram.size = size;

	/* Not sure why the performance is very poor if we keep the bo created
	 * by __xocl_create_bo_ioctl(). This is why we free it.
	 * To still reserve plram for descriptor, keep bo generated by
	 * xocl_drm_create_bo().
	 */
