...
static int axi_gate_level(struct platform_device *pdev)
{
	struct axi_gate *gate = platform_get_drvdata(pdev);
...
static struct xocl_axigate_funcs axigate_ops = {
	...
	.free = axi_gate_free,
	.get_level = axi_gate_level,
};

static int axi_gate_remove(struct platform_device *pdev)
{
...
static int axi_gate_probe(struct platform_device *pdev)
{
	struct axi_gate *gate;
	struct resource *res;
	int ret;
	...
	xocl_subdev_register(pdev, XOCL_SUBDEV_AXIGATE, &axigate_ops);
	return 0;

failed:
	axi_gate_remove(pdev);
	return ret;
}

struct platform_device_id axi_gate_id_table[] = {
	{ XOCL_DEVNAME(XOCL_AXIGATE), 0 },
	{ },
};

static struct platform_driver	axi_gate_driver = {
	.probe		= axi_gate_probe,
	.remove		= axi_gate_remove,
	.driver		= {
		.name = XOCL_DEVNAME(XOCL_AXIGATE),
	},
	.id_table = axi_gate_id_table,
};
