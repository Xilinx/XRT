 ...
 * generated by wiz.pl.
 */
const static struct xgq_vmr_clockwiz {
	/* target frequency */
	unsigned short ocl;
	/* config0 register */
...
/*
	static unsigned int xgq_get_clock_frequency_counter(struct platform_device *pdev *xgq, int idx)
	{
    u32 freq = 0;
    switch(idx){
    case 0:
        freq = xgq_get_data(pdev, FREQ_COUNTER_0);
			break;
    case 1:
        freq = xgq_get_data(pdev, FREQ_COUNTER_1);
			break;
    case 2:
        freq = xgq_get_data(pdev, FREQ_COUNTER_2);
			break;
    default:
        break;
    }
...
static int set_and_verify_freqs(struct platform_device *pdev,unsigned short *target_freqs, int num_freqs, int verify)
{
    ...
    for (i = 0; i < min(XGQ_CLOCK_WIZ_MAX_RES, num_freqs); ++i) {
		...
		lookup_freq = find_matching_freq(target_freqs[i]);
