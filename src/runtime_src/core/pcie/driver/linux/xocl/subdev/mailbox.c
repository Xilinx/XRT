	...
	struct device_attribute *attr, char *buf)
{
	...
	ret = mailbox_request(to_platform_device(dev), &req, sizeof(req) + sizeof(int32_t),
	...
	struct device_attribute *da, const char *buf, size_t count)
{
	...
	(void) mailbox_post_notify(mbx->mbx_pdev, &req, sizeof(req) + sizeof(int32_t));
...
static void mailbox_send_test_load_xclbin_kaddr(struct mailbox *mbx)
{
	...
	reqlen =  sizeof(req) + sizeof(int32_t) + data_len;
