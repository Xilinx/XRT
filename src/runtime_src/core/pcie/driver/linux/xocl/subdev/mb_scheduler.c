...
ert_cfg(struct xocl_ert *xert, unsigned int cq_size, unsigned int num_slots, bool cq_intr)
{
	...
	SCHED_DEBUGF("%s ert(%d) cq_size(%d) slots(num_slots) slot_size(%d) cq_intr(%d)\n",
...
exec_cfg_cmd(struct exec_core *exec, struct xocl_cmd *xcmd)
{
	...
	// If exec_cfg_cmd is called on new xclbin then by default
	// polling is true.  If configuration turns off polling, then
	// scheduler poll count must be decremented.
	bool exec_polling_save = exec->polling_mode;
	...
	// Polling may have toggled, correct scheduler count
	if (!exec->polling_mode && exec_polling_save)
 ...
 * is notified that some command has completed.  The command is 
 * removed from the running queue and freed for reuse.
 */
static void
...
exec_start_ctrl_cmd(struct exec_core *exec, struct xocl_cmd *xcmd)
{
	...
	// On configure, must configure KDS at a minimum
	if (cmd_opcode(xcmd) == ERT_CONFIGURE && exec_cfg_cmd(exec, xcmd)) {
		exec_mark_cmd_error(exec, xcmd);
		--exec->num_pending_cmds;
		cmd_free(xcmd);
		SCHED_DEBUGF("<- %s returns false for exec_cfg_cmd\n", __func__);
		return false;
	}

	// Let scheduler mode determine additional processing
...
#if 0
static void
scheduler_core_reset(struct xocl_scheduler *xs)
{
	struct list_head *pos, *next;
