 ...
 * Creates BO object in Xilinx FPGA; Export buffers to get FD.
 * Camera import's these buffers and will write to these buffers.
 ...
 * Compile command:
 * g++ -g -std=c++11 -I ../../include -I ../../drm/xocl xclgem6_camera.cpp util.cpp
 */
...
{
  std::string fname = "cameraFrameExport-" + std::to_string(frame) + ".jpg";
  //char* ff = fname.c_str();
  /*
...
static int runTest(int xilinx_fd)
{
  ...
  //Asks camera to change to DMA BUF mode; And Will import buffer FDs provided by user
  //It will not allocate any buffer inside camera device.
  ...
  std::cout << "CREATE BOs on Xilinx FPGA" << std::endl;
  xoclutil::TestBO bo0("bo0", xilinx_fd, image_size, nullptr);
  xoclutil::TestBO bo1("bo1", xilinx_fd, image_size, nullptr);
  xoclutil::TestBO bo2("bo2", xilinx_fd, image_size, nullptr);
  xoclutil::TestBO bo3("bo3", xilinx_fd, image_size, nullptr);
  xoclutil::TestBO bo4("bo4", xilinx_fd, image_size, nullptr);


  std::cout << "EXPORT Xilinx FPGA BOs to get their FDs" << std::endl;
  xilinx_bo_fd[0] = bo0.wexport();
  xilinx_bo_fd[1] = bo1.wexport();
  xilinx_bo_fd[2] = bo2.wexport();
  xilinx_bo_fd[3] = bo3.wexport();
  xilinx_bo_fd[4] = bo4.wexport();
