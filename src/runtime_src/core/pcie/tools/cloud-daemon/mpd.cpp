...
void Mpd::run()
{
    ...
    for (size_t i = 0; i < total; i++) {
        std::string sysfs_name = std::dynamic_pointer_cast<xrt_core::pci::pcidev_linux>(xrt_core::get_device_factory(i, true))->m_sysfs_name;
...
void Mpd::mpd_getMsg(size_t index)
{
    ...
    */
    ...
    std::dynamic_pointer_cast<xrt_core::pci::pcidev_linux>(xrt_core::get_device_factory(index))->m_sysfs_name.c_str());
}
