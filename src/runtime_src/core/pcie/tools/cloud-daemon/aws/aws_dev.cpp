...
static void awsPciRescan(int index)
{
    ...
    std::dynamic_pointer_cast<xrt_core::pci::pcidev_linux>(xrt_core::pci::get_dev(index))->sysfs_get("", "host_mem_size", err, hostmem_size, static_cast<uint64_t>(0));
    if (hostmem_size)
    ...
    while (dev_offline) {
        ...
        std::dynamic_pointer_cast<xrt_core::pci::pcidev_linux>(xrt_core::pci::get_dev(index))->sysfs_get<int>("", "dev_offline", err, dev_offline, -1);
    }
    if (!hostmem_size) {
        //no need to reconfig host mem, just tell the user xclbin load ioctl can be re-issued now
        std::dynamic_pointer_cast<xrt_core::pci::pcidev_linux>(xrt_core::pci::get_dev(index))->sysfs_put("", "dev_hotplug_done", err, 1);
        return;
    }
...
int AwsDev::awsLoadXclBin(const xclBin *buffer)
{
    ...
    if (imageInfoOld.spec.map[FPGA_APP_PF].device_id != imageInfoNew.ids.afi_device_ids.device_id ||
        ...
        std::string err;
        std::dynamic_pointer_cast<xrt_core::pci::pcidev_linux>(xrt_core::pci::get_dev(index))->sysfs_put("", "dev_hotplug_done", err, 0);
