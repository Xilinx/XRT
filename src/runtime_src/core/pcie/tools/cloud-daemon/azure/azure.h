...
public:
    ...
    static std::vector<std::string> get_serial_number()
    {
	    ...
	    size_t total = xrt_core::get_device_factory_total();
	    if (!total) {
        ...
        for (size_t i = 0; i < total; i++) {
            ...
            std::dynamic_pointer_cast<xrt_core::pci::pcidev_linux>(xrt_core::get_device_factory(i, true))->sysfs_get("xmc", "serial_num", errmsg, serialNumber);
	        if (!errmsg.empty() || !regex_match(serialNumber, sn)) {
           	    std::cerr << "azure warning(" << std::dynamic_pointer_cast<xrt_core::pci::pcidev_linux>(xrt_core::get_device_factory(i, true))->m_sysfs_name << ")";
