             ...
             cl_event *            event)
{
  ...
  // the table of allowed values for param_name for clGetDeviceInfo is
  // CL_FALSE).

  // CL_INVALID_OPERATION if clEnqueueWriteImage is called on image
  // which has been created with CL_MEM_HOST_READ_ONLY or
  // CL_MEM_HOST_NO_ACCESS.
  if (xocl(image)->get_flags() & (CL_MEM_HOST_READ_ONLY | CL_MEM_HOST_NO_ACCESS))
    throw xocl::error(CL_INVALID_OPERATION,"image buffer flags do now allow writing");
...
clEnqueueWriteImage(cl_command_queue     command_queue,
                    ...
                    cl_event *           event )
{
  validOrError(command_queue,image,blocking_write,origin,region,input_row_pitch,input_slice_pitch,ptr,
               num_events_in_wait_list,event_wait_list,event);

  if (!input_row_pitch)
    input_row_pitch = xocl(image)->get_image_bytes_per_pixel()*region[0];
  if (!input_slice_pitch && xocl(image)->get_image_slice_pitch())
  ...
  xocl::lop::set_event_action(uvent.get(), xocl::lop::action_write);
