...
MYINCLUDES := $(SDACCEL_RUNTIME_DIR) $(SDACCEL_RUNTIME_DIR)/driver/xclng/xrt/user_gem/
CPP_SUFFIX := .cpp
OBJS_PROPAGATE := no
...
ifeq ($(RDI_PLATFORM),lnx64)
 ...
 # release xbsak_gem platform/sw/bin dir
 $(foreach dir,$(XCLNG_RELEASE_DIRS),\
  $(eval $(call XCLNG_release_execs_rename_template,$(dir)/$(XCLNG_SUBDIR),$(EXEC),xbsak)))

 # same as above for internal
 # release xbsak_gem platform/sw/bin dir
 $(foreach dir,$(XCLNG_INTERNAL_RELEASE_DIRS),\
  $(eval $(call XCLNG_internal_release_execs_rename_template,$(dir)/$(XCLNG_SUBDIR),$(EXEC),xbsak)))
