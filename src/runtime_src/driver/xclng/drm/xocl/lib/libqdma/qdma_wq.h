...
struct qdma_wq {
	...
	int (*fp_done)(struct qdma_wqe *wqe);
};
...
{
	while (q->wq_unproc != q->wq_head &&
		(q->wq[q->wq_unproc].state == QDMA_WQE_STATE_CANCELED ||
		q->wq[q->wq_unproc].unproc_bytes == 0)) {
		q->wq_unproc++;
		q->wq_unproc &= q->wq_len - 1;
	}
	return (q->wq_unproc != q->wq_head) ? &q->wq[q->wq_unproc] : NULL;
}

static inline struct qdma_wqe *wq_next_pending(struct qdma_wq *q)
{
	while (q->wq_pending != q->wq_unproc &&
		(q->wq[q->wq_pending].state == QDMA_WQE_STATE_CANCELED ||
		q->wq[q->wq_pending].state == QDMA_WQE_STATE_DONE)) {
		q->wq_pending++;
		q->wq_pending &= q->wq_len - 1;
	}
	return (q->wq_pending != q->wq_unproc) ? &q->wq[q->wq_pending] : NULL;
}

static inline struct qdma_wqe *wq_next_free(struct qdma_wq *q)
{
	u32 next = ((q->wq_head + 1) & (q->wq_len - 1));
	u32 curr = q->wq_head;
