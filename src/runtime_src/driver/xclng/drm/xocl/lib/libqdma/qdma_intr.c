...
static void data_intr_direct(struct xlnx_dma_dev *xdev, int vidx, int irq)
{
	...
	list_for_each_entry(descq, &xdev->intr_list[vidx], intr_list)
		if (descq->conf.fp_descq_isr_top) {
			struct qdma_dev *qdev = xdev_2_qdev(xdev);
...
static void data_vector_handler(int irq, struct xlnx_dma_dev *xdev)
{
	...
	for (i = 0; i < xdev->num_vecs; i++) {
		if (xdev->msix[i].vector == irq) {
			xdev->intr_vec_map[i].intr_handler(i, irq,
							(void *)xdev);
			break;
		}
	}
...
{
	int rv = request_threaded_irq(xdev->msix[idx].vector, irq_top,
					irq_bottom, 0, xdev->mod_name, xdev);

	pr_info("%s requesting IRQ vector #%d: vec %d, type %d.\n",
		xdev->conf.name, idx, xdev->msix[idx].vector, type);
	...
	xdev->intr_vec_map[idx].intr_type = type;
	xdev->intr_vec_map[idx].intr_vec_index = idx;
	xdev->intr_vec_map[idx].intr_handler = handler;
...
int intr_ring_setup(struct xlnx_dma_dev *xdev)
{
	...
	if ((xdev->num_vecs != 0) && (xdev->num_vecs < xdev->conf.qsets_max)) {
		...
		for (counter = 0;
			counter < QDMA_DATA_VEC_PER_PF_MAX;
			counter++) {
			...
			bool c2h_upd_cmpl)
{
	...
	if (descq)
		qdma_descq_service_wb(descq, budget, c2h_upd_cmpl);
}
...
void qdma_enable_hw_err(struct xlnx_dma_dev *xdev, u8 hw_err_type)
{
	...
	case GLBL_ERR:
		__write_reg(xdev,
			QDMA_REG_GLBL_ERR_MASK,
			QDMA_REG_GLBL_ERR_MASK_VALUE);

		pr_debug("Global interrupts enabled: reg -> 0x%08x,  value =  0x%08x\n",
				QDMA_REG_GLBL_ERR_MASK,
				__read_reg(xdev, QDMA_REG_GLBL_ERR_MASK));
		break;
	case GLBL_DSC_ERR:
		__write_reg(xdev,
			QDMA_GLBL_DSC_ERR_MSK,
			QDMA_GLBL_DSC_ERR_MSK_VALUE);

		pr_debug("Global dsc interrupts enabled: reg -> 0x%08x,  value =  0x%08x\n",
				QDMA_GLBL_DSC_ERR_MSK,
				__read_reg(xdev, QDMA_GLBL_DSC_ERR_MSK));
		break;
	case GLBL_TRQ_ERR:
		__write_reg(xdev,
			QDMA_GLBL_TRQ_ERR_MSK,
			QDMA_GLBL_TRQ_ERR_MSK_VALUE);

		pr_debug("Global trq interrupts enabled: reg -> 0x%08x,  value =  0x%08x\n",
				QDMA_GLBL_TRQ_ERR_MSK,
				__read_reg(xdev, QDMA_GLBL_TRQ_ERR_MSK));
		break;
	case C2H_ERR:
		__write_reg(xdev,
			QDMA_REG_C2H_ERR_MASK,
			QDMA_REG_C2H_ERR_MASK_VALUE);

		pr_debug("C2H interrupts enabled: reg -> 0x%08x,  value =  0x%08x\n",
			QDMA_REG_C2H_ERR_MASK,
			__read_reg(xdev, QDMA_REG_C2H_ERR_MASK));
		break;
	case C2H_FATAL_ERR:
		__write_reg(xdev,
			QDMA_C2H_FATAL_ERR_MASK,
			QDMA_C2H_FATAL_ERR_MASK_VALUE);

		pr_debug("C2H fatal interrupts enabled: reg -> 0x%08x,  value =  0x%08x\n",
			QDMA_C2H_FATAL_ERR_MASK,
			__read_reg(xdev, QDMA_C2H_FATAL_ERR_MASK));
		break;
	case H2C_ERR:
		...
		pr_debug("C2H interrupts enabled: reg -> 0x%08x,  value =  0x%08x\n",
			QDMA_H2C_ERR_MASK,
			__read_reg(xdev, QDMA_H2C_ERR_MASK));
		break;
	default:
		__write_reg(xdev,
			QDMA_REG_GLBL_ERR_MASK,
...
int get_intr_ring_index(struct xlnx_dma_dev *xdev, u32 vector_index)
{
	...
	ring_index = (vector_index - xdev->dvec_start_idx) +
			(xdev->func_id * QDMA_DATA_VEC_PER_PF_MAX);
