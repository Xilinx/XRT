...
struct xocl_mig_cache_funcs {
	int (*get_data)(struct platform_device *pdev, void *buf);
};
#define	MIG_CACHE_DEV(xdev)	SUBDEV(xdev, XOCL_SUBDEV_MIG_CACHE).pldev
#define	MIG_CACHE_OPS(xdev)							\
	((struct xocl_mig_cache_funcs *)SUBDEV(xdev, XOCL_SUBDEV_MIG_CACHE).ops)
#define	MIG_CACHE_CB(xdev)	\
	(MIG_CACHE_DEV(xdev) && MIG_CACHE_OPS(xdev))
#define	xocl_mig_cache_get_data(xdev, buf)				\
	(MIG_CACHE_CB(xdev) ?						\
	MIG_CACHE_OPS(xdev)->get_data(MIG_CACHE_DEV(xdev), buf) : \
	0)
