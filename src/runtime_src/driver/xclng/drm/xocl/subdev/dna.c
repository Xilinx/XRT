...
static void get_xlnx_dna_prop(struct device *dev, enum dna_kind kind, void *val)
{
	...
	char *buf)
{
	...
	get_xlnx_dna_prop(dev, STATUS, &status);
	...
	char *buf)
{
	...
	get_xlnx_dna_prop(dev, DNA_RAW, dna);
	...
	char *buf)
{
	...
	get_xlnx_dna_prop(dev, CAP, &capability);
	return sprintf(buf, "0x%x\n", capability);
}
	...
	char *buf)
{
	...
	get_xlnx_dna_prop(dev, VER, &version);
	...
	char *buf)
{
	...
	get_xlnx_dna_prop(dev, REVERSION, &revision);
...
static void dna_get_data(struct platform_device *pdev, void *buf)
{
	...
	get_xlnx_dna_prop(&pdev->dev, STATUS, &dna_status.status);
	get_xlnx_dna_prop(&pdev->dev, CAP, &dna_status.capability);
	get_xlnx_dna_prop(&pdev->dev, DNA_RAW, dna_status.dna);
	get_xlnx_dna_prop(&pdev->dev, VER, &dna_status.dna_version);
	get_xlnx_dna_prop(&pdev->dev, REVERSION, &dna_status.revision);
