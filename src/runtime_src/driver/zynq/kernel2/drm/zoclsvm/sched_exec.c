...
{
  struct drm_zoclsvm_dev *zdev = cmd->ddev->dev_private;
 ...
 */
static int
add_cmd(struct drm_device *dev, struct drm_zoclsvm_bo* bo)
{
...
reset_exec(struct sched_exec_core* exec)
{
  ...
  list_for_each_safe(pos, next, &pending_cmds) {
    struct sched_cmd *cmd = list_entry(pos, struct sched_cmd, list);
    struct drm_zoclsvm_dev *zdev = cmd->ddev->dev_private;
    if (zdev->exec != exec)
      continue;
    DRM_INFO("deleting stale pending cmd\n");
    drm_gem_object_unreference_unlocked(&cmd->bo->base);
    recycle_cmd(cmd);
  }
...
/**
 * zoclsvm_execbuf_ioctl() - Entry point for exec buffer.
 *
 ...
 * Return: 0 on success, -errno otherwise
 */
int zoclsvm_execbuf_ioctl(struct drm_device *dev,
                       void *data,
                       struct drm_file *filp)
{
  struct drm_gem_object *gem_obj;
  struct drm_zoclsvm_bo *zocl_bo;
  struct drm_zoclsvm_dev *zdev = dev->dev_private;
  struct drm_zocl_execbuf *args = data;
  int ret = 0;

  SCHED_DEBUG("-> zoclsvm_execbuf_ioctl\n");
  gem_obj = zoclsvm_gem_object_lookup(dev, filp, args->exec_bo_handle);
  if (!gem_obj) {
    DRM_ERROR("Failed to look up GEM BO %d\n", args->exec_bo_handle);
    return -EINVAL;
  }

  zocl_bo = to_zoclsvm_bo(gem_obj);
  if (!zoclsvm_bo_execbuf(zocl_bo)) {
    ret = -EINVAL;
    goto out;
  }
...
out:
  drm_gem_object_unreference_unlocked(&zocl_bo->base);
  return ret;
}
