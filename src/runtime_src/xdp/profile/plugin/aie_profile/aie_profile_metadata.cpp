...
namespace xdp {
      ...
      : deviceID(deviceID), handle(handle)
  {
    ...
    for (int module = 0; module < NUM_MODULES; ++module) {
      ...
      if (type == module_type::shim)
        getConfigMetricsForInterfaceTiles(module, metricsSettings,
                                          graphMetricsSettings);
      else
        getConfigMetricsForTiles(module, metricsSettings, graphMetricsSettings,
                                 type);
    }
  }
  ...
  void AieProfileMetadata::checkSettings()
  {
    ...
    const std::set<std::string> validSettings{
        ...
        "tile_based_aie_memory_metrics",     "tile_based_memory_tile_metrics",
    ...
    const std::map<std::string, std::string> deprecatedSettings{
        {"aie_profile_core_metrics",
         "AIE_profile_settings.graph_based_aie_metrics or "
         "tile_based_aie_metrics"},
        {"aie_profile_memory_metrics",
         "AIE_profile_settings.graph_based_aie_memory_metrics or "
    ...
    auto tree1 =
        xrt_core::config::detail::get_ptree_value("AIE_profile_settings");
    for (ptree::iterator pos = tree1.begin(); pos != tree1.end(); pos++) {
      if (validSettings.find(pos->first) == validSettings.end()) {
        std::stringstream msg;
        msg << "The setting AIE_profile_settings." << pos->first
            << " is not recognized. "
  ...
  int AieProfileMetadata::getHardwareGen()
  {
    ...
    if (!gotValue) {
      ...
      if (!data.first || !data.second) {
        ...
        hwGen = aie_meta.get_child("aie_metadata.driver_config.hw_gen")
                    .get_value<int>();
      }
      gotValue = true;
    }
  ...
  uint16_t AieProfileMetadata::getAIETileRowOffset()
  {
    ...
    if (!gotValue) {
      ...
      if (!data.first || !data.second) {
        ...
        read_aie_metadata(data.first, data.second, aie_meta);
        rowOffset =
            aie_meta.get_child("aie_metadata.driver_config.aie_tile_row_start")
                .get_value<uint16_t>();
      }
      gotValue = true;
    }
  ...
  std::vector<std::string> AieProfileMetadata::getSettingsVector(
      std::string settingsString)
  {
    if (settingsString.empty()) return {};
  ...
  std::vector<tile_type> AieProfileMetadata::get_interface_tiles(
      const xrt_core::device* device, const std::string& metricStr,
      int16_t channelId, bool useColumn, uint32_t minCol, uint32_t maxCol)
  {
    ...
    for (auto& plio : plios) {
      ...
      if ((channelId >= 0) && (channelId != streamId)) continue;
    ...
    if ((plioCount == 0) && (channelId >= 0)) {
      std::string msg = "No tiles used channel ID " +
  ...
  std::vector<tile_type> AieProfileMetadata::get_mem_tiles(
      const xrt_core::device* device, const std::string& graph_name,
      const std::string& kernel_name)
  {
    if (getHardwareGen() == 1) return {};
    ...
    auto sharedBufferTree = aie_meta.get_child_optional(
        "aie_metadata.TileMapping.SharedBufferToTileMapping");
    if (!sharedBufferTree) return {};
    ...
    for (auto const& shared_buffer : sharedBufferTree.get()) {
      ...
      if (kernel_name.compare("all") != 0) {
        ...
        std::string functionStr =
            shared_buffer.second.get<std::string>("function");
    ...
    std::unique_copy(allTiles.begin(), allTiles.end(),
                     std::back_inserter(memTiles), tileCompare);
    return memTiles;
  }

  inline void throw_if_error(bool err, const char* msg)
  {
    if (err) throw std::runtime_error(msg);
  }

  std::vector<tile_type> AieProfileMetadata::get_event_tiles(
      const xrt_core::device* device, const std::string& graph_name,
      module_type type)
  {
    auto data = device->get_axlf_section(AIE_METADATA);
    ...
    if (type == module_type::shim) return {};

    const char* col_name =
        (type == module_type::core) ? "core_columns" : "dma_columns";
    const char* row_name =
        (type == module_type::core) ? "core_rows" : "dma_rows";
    ...
    for (auto& graph : aie_meta.get_child("aie_metadata.EventGraphs")) {
      ...
      for (auto& node : graph.second.get_child(row_name))
        tiles.at(count++).row =
            static_cast<uint16_t>(std::stoul(node.second.data())) + rowOffset;
  ...
  // all)
  std::vector<tile_type> AieProfileMetadata::get_aie_tiles(
      const xrt_core::device* device, const std::string& graph_name,
      module_type type)
  {
    std::vector<tile_type> tiles;
    ...
    if (type == module_type::dma) {
      ...
      std::unique_copy(dmaTiles.begin(), dmaTiles.end(), back_inserter(tiles),
                       tileCompare);
    }
    return tiles;
  }

  std::vector<tile_type> AieProfileMetadata::get_tiles(
      const xrt_core::device* device, const std::string& graph_name,
      module_type type, const std::string& kernel_name)
  {
    ...
    auto kernelToTileMapping = aie_meta.get_child_optional(
        "aie_metadata.TileMapping.AIEKernelToTileMapping");
    if (!kernelToTileMapping) return {};
  ...
  void AieProfileMetadata::getConfigMetricsForTiles(
      int moduleIdx, const std::vector<std::string>& metricsSettings,
      const std::vector<std::string>& graphMetricsSettings,
      const module_type mod)
  {
    if ((metricsSettings.empty()) && (graphMetricsSettings.empty())) return;
    ...
    uint16_t rowOffset =
        (mod == module_type::mem_tile) ? 1 : getAIETileRowOffset();
    auto modName =
    ...
    std::unique_copy(validTilesVec.begin(), validTilesVec.end(),
                     std::inserter(allValidTiles, allValidTiles.end()),
                     tileCompare);
    ...
    std::vector<std::vector<std::string>> graphMetrics(
        graphMetricsSettings.size());

    // Graph Pass 1 : process only "all" metric setting
    for (size_t i = 0; i < graphMetricsSettings.size(); ++i) {
      // Split done only in Pass 1
      boost::split(graphMetrics[i], graphMetricsSettings[i],
                   boost::is_any_of(":"));

      // Check if graph is not all or if invalid kernel
      if (graphMetrics[i][0].compare("all") != 0) continue;
      if ((graphMetrics[i][1].compare("all") != 0) &&
          (std::find(allValidKernels.begin(), allValidKernels.end(),
                     graphMetrics[i][1]) == allValidKernels.end())) {
        std::stringstream msg;
        msg << "Kernel " << graphMetrics[i][1] << " not found. The graph_based_"
            << modName << "_metrics setting " << graphMetricsSettings[i]
            << " will be ignored.";
        xrt_core::message::send(severity_level::warning, "XRT", msg.str());
        continue;
      }
      ...
      auto tiles =
          get_tiles(device.get(), graphMetrics[i][0], mod, graphMetrics[i][1]);
      for (auto& e : tiles) {
      ...
      if (graphMetrics[i].size() == 5) {
        try {
          for (auto& e : tiles) {
            configChannel0[e] =
                static_cast<uint8_t>(std::stoul(graphMetrics[i][3]));
            configChannel1[e] =
                static_cast<uint8_t>(std::stoul(graphMetrics[i][4]));
          }
        } catch (...) {
          std::stringstream msg;
    ...
    for (size_t i = 0; i < graphMetricsSettings.size(); ++i) {
      ...
      if (graphMetrics[i][0].compare("all") == 0) continue;
      if (std::find(allValidGraphs.begin(), allValidGraphs.end(),
                    graphMetrics[i][0]) == allValidGraphs.end()) {
        std::stringstream msg;
        msg << "Graph " << graphMetrics[i][0] << " not found. The graph_based_"
            << modName << "_metrics setting " << graphMetricsSettings[i]
            << " will be ignored.";
        xrt_core::message::send(severity_level::warning, "XRT", msg.str());
        continue;
    ...
    for (size_t i = 0; i < metricsSettings.size(); ++i) {
      if ((metrics[i].size() != 3) && (metrics[i].size() != 5)) continue;
    ...
    for (size_t i = 0; i < metricsSettings.size(); ++i) {
      ...
      if (metrics[i].size() == 4) {
        try {
          configChannel0[tile] =
              static_cast<uint8_t>(std::stoul(metrics[i][2]));
          configChannel1[tile] =
              static_cast<uint8_t>(std::stoul(metrics[i][3]));
        } catch (...) {
          std::stringstream msg;
          msg << "Channel specifications in tile_based_" << modName
              << "_metrics are not valid and hence ignored.";
    ...
    for (auto& tileMetric : configMetrics[moduleIdx]) {
      ...
      if (tileMetric.second.empty() ||
          (tileMetric.second.compare("off") == 0)) {
        offTiles.push_back(tileMetric.first);
        continue;
      }

      // Ensure requested metric set is supported (if not, use default)
      if (std::find(metricStrings[mod].begin(), metricStrings[mod].end(),
                    tileMetric.second) == metricStrings[mod].end()) {
        if (showWarning) {
          std::stringstream msg;
          msg << "Unable to find " << moduleNames[moduleIdx] << " metric set "
              << tileMetric.second << ". Using default of " << defaultSet
              << ".";
          xrt_core::message::send(severity_level::warning, "XRT", msg.str());
          showWarning = false;
        }
  ...
  void AieProfileMetadata::getConfigMetricsForInterfaceTiles(
      int moduleIdx, const std::vector<std::string>& metricsSettings,
      const std::vector<std::string> graphMetricsSettings)
  {
    if ((metricsSettings.empty()) && (graphMetricsSettings.empty())) return;
    ...
    for (size_t i = 0; i < metricsSettings.size(); ++i) {
      ...
      if (metrics[i][0].compare("all") != 0) continue;

      int16_t channelId =
          (metrics[i].size() < 3)
              ? -1
              : static_cast<uint16_t>(std::stoul(metrics[i][2]));
    ...
    for (auto& tileMetric : configMetrics[moduleIdx]) {
      ...
      if (std::find(metricVec.begin(), metricVec.end(), tileMetric.second) ==
          metricVec.end()) {
        if (showWarning) {
          std::string msg = "Unable to find interface_tile metric set " +
                            tileMetric.second + ". Using default of " +
                            defaultSet + ". ";
          xrt_core::message::send(severity_level::warning, "XRT", msg);
          showWarning = false;
        }
  ...
  void AieProfileMetadata::read_aie_metadata(const char* data, size_t size,
                                             pt::ptree& aie_project)
  {
  ...
  uint8_t AieProfileMetadata::getMetricSetIndex(std::string metricString,
                                                module_type mod)
  {
    auto stringVector = metricStrings[mod];

    auto itr =
        std::find(stringVector.begin(), stringVector.end(), metricString);
