...
def extract_variables_from_cpp():
    ...
    try:
        ...
        #Change filename according to generation specific register .h file
...
def write_to_log_file(variables):
    ...
    if variables:
        ...
        with open("py_rev_map_gen2.txt", "w") as log_file:
            #for line in variables:
            for var_name, hex_value in variables.items():
                #log_file.write('{{"{}": "{}"}},\n'.format(var_name, hex_value))  # Write each variable on a new line
                if 0 : #line.startswith('{"None"'):
                    continue
                else:
                    #log_file.write(line + ",\n")  # Write each variable on a new line
                    log_file.write('{{{}: "{}"}},\n'.format(hex_value,var_name))  # Write each variable on a new line
        print("Dictionary has been written to py_rev_map_gen2.txt")
