...
namespace xdp {
  ...
  NOCPlugin::NOCPlugin() : XDPPlugin()
  {
    ...
    writers.push_back(new HALHostTraceWriter("noc_profile.csv",
                      version, creationTime, xrtVersion, toolVersion));

    (db->getStaticInfo()).addOpenedFile("noc_profile.csv", "NOC_PROFILE");
  }
  ...
  void NOCPlugin::writeAll(bool openNewFiles)
  {
    for (auto w : writers) {
