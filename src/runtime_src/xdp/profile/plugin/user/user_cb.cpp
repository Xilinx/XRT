...
namespace xdp {
  ...
  static void user_event_start_cb(unsigned int functionID, const char* label, const char* tooltip)
  {
    ...
    VTFEvent* event = new UserRange(
        0, static_cast<double>(timestamp),
        true, // isStart
        (db->getDynamicInfo()).addString(labelStr), (db->getDynamicInfo()).addString(tooltipStr));
    (db->getDynamicInfo()).addEvent(event);
    (db->getDynamicInfo()).markStart(functionID, event->getEventId());

    // Record information for statistics
    std::pair<const char*, const char*> desc = std::make_pair(labelStr, tooltipStr);
...
extern "C" void user_event_start_cb(unsigned int functionID, const char* label, const char* tooltip)
{
  xdp::user_event_start_cb(functionID, label, tooltip);
}

extern "C" void user_event_end_cb(unsigned int functionID)
{
  xdp::user_event_end_cb(functionID);
}

extern "C" void user_event_happened_cb(const char* label)
{
  xdp::user_event_happened_cb(label);
}

extern "C" void user_event_time_ns_cb(unsigned long long int time_ns, const char* label)
{
