...
void NOC::parseName(std::string name)
{
  ...
  try {mMasterName = result.at(0);} catch(std::exception& e) {mMasterName = "";}
  try {mCellName = result.at(1);} catch(std::exception& e) {mCellName = "";}
  try {mReadQos = std::stoull(result.at(2));} catch(std::exception& e) {mReadQos = 0;}
  try {mWriteQos = std::stoull(result.at(3));} catch(std::exception& e) {mWriteQos = 0;}
  try {mNpiClockFreqMhz = std::stod(result.at(4));} catch(std::exception& e) {mNpiClockFreqMhz = 299.997009;}
}
