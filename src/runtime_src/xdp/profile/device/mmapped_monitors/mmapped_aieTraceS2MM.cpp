...
#include "mmapped_aieTraceS2MM.h"

namespace xdp {

MMappedAIETraceS2MM::MMappedAIETraceS2MM(Device* handle, uint64_t index, uint64_t instIdx, debug_ip_data* data)
                : AIETraceS2MM(handle, index, data),
                  instance_index(instIdx)
{
  // Open AIETraceS2MM Device Driver File
...
MMappedAIETraceS2MM::~MMappedAIETraceS2MM()
{
  munmap(mapped_device, PROFILE_IP_SZ);
  close(driver_FD);
}

bool MMappedAIETraceS2MM::isMMapped()
{
...
int MMappedAIETraceS2MM::read(uint64_t offset, size_t size, void* data)
{
  if(!isMMapped()) {
    return 0;
  }
...
int MMappedAIETraceS2MM::write(uint64_t offset, size_t size, void* data)
{
  if(!isMMapped()) {
    return 0;
  }
