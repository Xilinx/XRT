...
#include "mmapped_add.h"

namespace xdp {

MMappedDeadlockDetector::MMappedDeadlockDetector(Device* handle, uint64_t index, debug_ip_data* data)
                    : DeadlockDetector(handle, index, data)
{
  // Open DeadlockDetector Device Driver File
  std::string subDev("deadlock_detector");
...
MMappedDeadlockDetector::~MMappedDeadlockDetector()
{
