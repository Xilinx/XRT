...
void get_cu_start(const xrt::command* cmd, const xocl::execution_context* ctx)
{
  ...
  auto rtp = XCL::RTSingleton::Instance()->getProfileManager();
  rtp->logKernelExecution(objId, programId, eventId, XCL::RTProfile::START, kname, xname, contextId,
...
void get_cu_done(const xrt::command* cmd, const xocl::execution_context* ctx)
{
  ...
  rtp->logKernelExecution(objId, programId, eventId, XCL::RTProfile::END, kname, xname, contextId,
                          commandQueueId, deviceName, deviceId, globalWorkDim,
                          workGroupSize, localWorkDim, cuName);
}
...
// Check if a given platform contains a unified device
// NOTE: starting in 2017.4 runtime, this is always true
bool
is_unified(key k)
{
  return true;
...
set_profile_num_slots(key k, xclPerfMonType type, unsigned numSlots)
{
  ...
  for (auto device : platform->get_device_range())
    xdp::profile::device::setProfileNumSlots(device, type, numSlots);
  return 0;
}
...
log_device_trace(key k, xclPerfMonType type, bool forceRead)
{
  ...
  if (mgr->getLoggingTrace(type)) {
    //XCL::logprintf("Trace already being logged (type=%d)\n", type);
    return -1;
  }
  ...
  if (isValidPerfMonTypeTrace(k,type)) {
    ...
    typedef std::chrono::duration<uint64_t, std::ratio<1, 1000000>> duration_us;
    duration_us time_span = std::chrono::duration_cast<duration_us>(end - start);
    mgr->setLoggingTraceUsec(mgr->getLoggingTraceUsec + time_span.count());
#endif
  }
  return ret;
}
...
startTrace(key k, xclPerfMonType type, size_t numComputeUnits)
{
  ...
  auto data = get_data(k);
  auto profileMgr = XCL::RTSingleton::Instance()->getProfileManager();
  ...
  XCL::RTProfile::e_device_trace deviceTrace = profileMgr->getTransferTrace();
  XCL::RTProfile::e_stall_trace stallTrace = profileMgr->getStallTrace();
  uint32_t traceOption = (deviceTrace == XCL::RTProfile::DEVICE_TRACE_COARSE) ? 0x1 : 0x0;
  if (deviceTrace != XCL::RTProfile::DEVICE_TRACE_OFF) traceOption   |= (0x1 << 1);
  if (stallTrace & XCL::RTProfile::STALL_TRACE_INT)    traceOption   |= (0x1 << 2);
  if (stallTrace & XCL::RTProfile::STALL_TRACE_STR)    traceOption   |= (0x1 << 3);
  if (stallTrace & XCL::RTProfile::STALL_TRACE_EXT)    traceOption   |= (0x1 << 4);
...
startCounters(key k, xclPerfMonType type)
{
  ...
  double deviceClockMHz = xdevice->getDeviceClock().get();
  if (deviceClockMHz > 0)
    XCL::RTSingleton::Instance()->getProfileManager()->setDeviceClockFreqMHz( deviceClockMHz );

  xdevice->startCounters(type);
  data->mSampleIntervalMsec =
    XCL::RTSingleton::Instance()->getProfileManager()->getSampleIntervalMsec();
  return CL_SUCCESS;
}
...
logTrace(key k, xclPerfMonType type, bool forceRead)
{
  ...
  if (forceRead || (numSamples > data->mSamplesThreshold)) {
    ...
    while (1)
    {
      ...
      XCL::RTSingleton::Instance()->getProfileManager()->logDeviceTrace(device_name, binary_name, type, data->mTraceVector);
