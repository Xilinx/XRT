...
XRT API Trace
   Run failing application with XRT logging enabled in :doc:`xrt_ini.rst` ::
...
If an application is producing incorrect results it is important to review the host code to ensure that host application and xclbin agree on memory topology. One way to validate this at runtime is to enable XRT logging in :doc:`xrt_ini.rst` and then carefully go through all buffer allocation requests.
