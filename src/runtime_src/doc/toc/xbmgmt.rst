...
- The ``--image`` option is used with ``--base`` partition if multiple base partition packages are installed in the system. The specific base parition can be specified by the name (or name with full-path)
