...
run(xrt::device* mydev)
{
  ...
  xrt::hal::BufferObjectHandle bo = mydev->alloc(bufSize);
  char *data1 = new char[bufSize];
  char *data2 = new char[bufSize];

  std::memset(data1, randomChar, bufSize);
  std::memset(data2, 0, bufSize);
  xrt::event ev1 = mydev->write(bo, data1, bufSize, 0);
  ev1.wait();
  xrt::event ev2 = mydev->read(bo, data2, bufSize, 0);
  ev1.wait();
  int result = std::memcmp(data1, data2, bufSize);
  BOOST_CHECK_EQUAL(result, 0);

  std::memset(data2, 0, bufSize);
  xrt::event ev3 = mydev->sync(bo, bufSize, 0, xrt::device::direction::HOST2DEVICE);
  ev3.wait();
  xrt::event ev4 = mydev->write(bo, data2, bufSize, 0);
  ev4.wait();
  ev4 = mydev->sync(bo, bufSize, 0, xrt::device::direction::DEVICE2HOST);
  ev4.wait();
  ...
  std::memset(data1, randomChar, bufSize);
  xrt::event ev5 = mydev->sync(bo, bufSize, 0, xrt::device::direction::HOST2DEVICE);
  ev5.wait();
  std::memset(data3, 0, bufSize);
  xrt::event ev6 = mydev->sync(bo, bufSize, 0, xrt::device::direction::DEVICE2HOST);
  ev6.wait();
  xrt::event ev7 = mydev->read(bo, data2, bufSize, 0);
  ev7.wait();
