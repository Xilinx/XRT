...
class BoostLibrary:
  ...
  def __init__(self, skip):
    ...
    self.tag_version = "boost-1.75.0"
    self.library_version = "1_75"
  ...
  def getBuildAndInstallLibraryMinimum(self, verbose):
    ...
    cmd = "b2 -j6 install toolset=vc141 --prefix=" + self.install_dir + " --build-type=complete address-model=64 architecture=x86 link=static threading=multi --with-filesystem --with-program_options --with-system"
    print (cmd)
    os.system(cmd)
  ...
  def getBuildAndInstallLibrary(self, verbose):
    ...
    cmd = "b2 -j6 install toolset=vc141 --prefix=" + self.install_dir + " --build-type=complete"
    print (cmd)
    os.system(cmd)
