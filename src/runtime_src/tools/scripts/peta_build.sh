...
install_recipes()
{
	...
	if [ $? != 0 ]; then
		...
		echo "EXTERNALSRC = \"$XRT_REPO_DIR/src/runtime_src/driver/zynq/drm/zocl\"" >> $ZOCL_BB
		echo "EXTERNALSRC_BUILD = \"$XRT_REPO_DIR/src/runtime_src/driver/zynq/drm/zocl\"" >> $ZOCL_BB
	fi
...
CPU_ARCH="a53"
GLOB_DTS=${XRT_REPO_DIR}/src/runtime_src/driver/zynq/fragments/xlnk_dts_fragment_mpsoc.dts
...
if [ "X$TEMPLATE" == "Xzynq" ]; then
	...
	CPU_ARCH="a9"
	GLOB_DTS=${XRT_REPO_DIR}/src/runtime_src/driver/zynq/fragments/xlnk_dts_fragment_zynq.dts
fi
