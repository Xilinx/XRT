...
namespace xrt::core::hip
{
  ...
  private:
    ...
    /**
     * @brief get last error without lock
     * @return Last device HIP error code
     */
    hipError_t get_last_error_nolock();

    /**
     * @brief update the last async error stored in HIP
     * This function is called to update the m_hip_last_error without acquiring a lock.
    ...
    std::mutex m_lock; // lock for last error access control
  }; // class error
