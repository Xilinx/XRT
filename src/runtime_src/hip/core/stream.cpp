...
await_completion()
{
  ...
  while(!m_cmd_queue.empty()) {
    ...
    try {
      ...
      std::string err_str = "CMD[" + std::to_string(cmd_id) + "]:" + get_untangled_type_name(*cmd) + ":" + ex.what();
      xrt::core::hip::error::instance().record_local_error(hipErrorLaunchFailure, err_str);
      has_failure = true;
    }
    catch (...) {
      std::string err_str = "CMD[" + std::to_string(cmd_id) + "]:" + get_untangled_type_name(*cmd) + ":unknown exception";
      xrt::core::hip::error::instance().record_local_error(hipErrorLaunchFailure, err_str);
      has_failure = true;
    }
