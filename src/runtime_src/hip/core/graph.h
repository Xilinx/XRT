...
public:
  ...
  // The child is stored as a weak_ptr in the parent's m_children vector to avoid strong reference cycle dependencies.
  void
  add_dep_node(std::shared_ptr<graph_node> parent)
  {
    m_deps_list.push_back(parent);
...
{
private:
  std::vector<std::shared_ptr<command>> m_cmd_exec_list;
