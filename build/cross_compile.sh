...
time env DKMS_FLOW=yes $CMAKE -DXRT_EDGE=1 -DCMAKE_BUILD_TYPE=Release -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DCMAKE_TOOLCHAIN_FILE=$BUILDDIR/$toolchain -Daarch=$AARCH -Dsysroot:PATH=$SYSROOT -Dflavor=$OS_TARGET -Dversion=$VERSION -DCROSS_COMPILE=yes -DCMAKE_C_COMPILER=$c_compiler -DCMAKE_CXX_COMPILER=$cxx_compiler ../../src
